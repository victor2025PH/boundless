...
| chengjie 智聊 | 117 机 `D:\chengjie-instances`（本仓 engines/chengjie 是旧快照）；GitHub 空私仓 **zhiliao** 已备，待迁入 | 117 的 Cursor，联络单沟通 |
...
- ✅ engines/avatarhub 死快照已清退（留指路 README）；engines/huoke 已迁独立仓 `victor2025PH/huoke`；**engines/chengjie 去留待 117 回信**（联络单 ASK_from_zhongshu_20260817），回信前保持原样。
...
- 其他机器的克隆（117 `D:\boundless`+`D:\boundless-prod`、198 `D:\boundless`）：部署克隆只 pull 不 commit；117 的 sprint01 分支（5 未推送/803 脏）已在联络单里提醒收敛。
