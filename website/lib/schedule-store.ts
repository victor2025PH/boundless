...
export async function runDuePosts(): Promise<{ ran: number }> {
  ...
  try {
    ...
    for (const post of due) {
      ...
      const res = await broadcastMessage({ text: post.text, target: post.target, withButton: post.withButton, campaign: `sched-${day}` });
      ran += 1;
      await serialize(async () => {
        ...
        if (res.ok) {
          ...
          p.error = undefined;
          await recordPublish({ kind: "scheduled", target: post.target, summary: post.text, campaign: `sched-${day}` });
        } else {
          p.status = "failed";
