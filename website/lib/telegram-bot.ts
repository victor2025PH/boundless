...
export function richLinkKeyboard(lang: BotLang): InlineBtn[][] {
  ...
  return [
    [
      { text: site, url: siteUtmLink("bot", "links") },
...
export function faqListKeyboard(lang: BotLang) {
  ...
  rows.push([
    ...
    { text: lang === "zh" ? "打开官网" : "Open site", web_app: { url: siteUtmLink("bot", "faq") } },
  ]);
  return rows;
}
...
/** 发文件（multipart，不能走 JSON 的 tgCall）；诊断包直传客服会话用。≤50MB（Bot API 上限）。 */
export async function sendDocument(
  ...
  fd.append("document", new Blob([new Uint8Array(data)], { type: "application/zip" }), filename);
...
export async function setupBot(opts?: { skipWebhook?: boolean }) {
  ...
  await run("setMyCommands", "setMyCommands", {
    commands: [
      ...
      { command: "services", description: "业务能力 · 三系八产品 / Solutions" },
