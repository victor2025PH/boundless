    ...
    <section id="engine" className="relative overflow-hidden py-24">
      ...
      <div className="mx-auto max-w-7xl px-5">
              ...
              <Reveal key={it.title.en} delay={(idx % 3) * 0.05}>
                <div className="card-hover group relative flex h-full flex-col overflow-hidden rounded-2xl border border-white/10 bg-white/[0.03] p-5">
                  <div className="mb-4 flex items-center justify-between">
                    ...
                    {it.badge && (
                      <span className="rounded-full border border-neon-violet/40 bg-neon-violet/10 px-2 py-0.5 text-[11px] font-semibold text-neon-violet">
                        {it.badge[lang]}
                      </span>
                    )}
                  </div>
                  <span className="text-xs font-medium text-neon-cyan/80">{it.line[lang]}</span>
                  ...
                  <p className="proof-pill mt-4 inline-flex w-fit items-center gap-1.5 rounded-lg border border-white/10 bg-black/30 px-2.5 py-1 font-mono text-xs text-emerald-300/90">
        ...
        <Reveal className="mt-8 text-center">
          <p className="mx-auto max-w-3xl text-xs leading-relaxed text-slate-500">{c.footnote[lang]}</p>
          <a
            href={CONTACT_URL}
            target="_blank"
            rel="noreferrer"
