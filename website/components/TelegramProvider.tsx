...
export function TelegramProvider({ children }: { children: ReactNode }) {
  ...
  useEffect(() => {
    ...
    // deep link from bot ?startapp=pricing or start_param（"__" 后是 campaign 归因段，路由时去掉）
    const spEntry = sp ? sp.split("__")[0] : null;
    const target = spEntry ? START_MAP[spEntry] ?? spEntry : null;
    if (target) {
