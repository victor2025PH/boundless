...
export default function ParticleField() {
  ...
  useEffect(() => {
    ...
    // 低配挡位(layout 内联脚本判定)直接跳过 O(n²) 粒子连线,省 CPU/GPU
    const lowTier = document.documentElement.getAttribute("data-fx") === "low";
    if (reduced || coarse || small || lowTier) return;
  ...
  return (
    <canvas
      ref={canvasRef}
      aria-hidden
      className="particle-canvas pointer-events-none fixed inset-0 -z-10 h-full w-full opacity-70"
    />
  );
}
