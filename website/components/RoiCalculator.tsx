          ...
          <div className="grid gap-6 rounded-3xl border border-white/10 bg-ink-900/60 p-6 lg:grid-cols-2 lg:p-8">
            ...
            <div className="space-y-5">
              ...
              <div className="rounded-xl border border-white/5 bg-ink-950/40 p-3">
                <p className="mb-1.5 text-xs font-medium text-slate-400">{r.assumptionsTitle}</p>
                <ul className="space-y-1">
                  {r.assumptions.map((a) => (
                    <li key={a} className="flex items-start gap-1.5 text-xs leading-relaxed text-slate-500">
            ...
            <div className="flex flex-col">
              <div className="rounded-2xl border border-neon-cyan/30 bg-gradient-to-br from-neon-cyan/10 to-neon-violet/10 p-6 text-center">
                ...
                <div className="mt-5 space-y-2 text-left">
                  <div className="flex items-center gap-2">
                    <span className="w-16 shrink-0 text-xs text-slate-400">{r.planLabel}</span>
                    <div className="h-2.5 flex-1 overflow-hidden rounded-full bg-white/10">
                      <div className="h-full rounded-full bg-slate-500" style={{ width: `${calc.inBar}%` }} />
                    </div>
                    <span className="w-20 shrink-0 text-right text-xs tabular-nums text-slate-400">{fmt(calc.planCost)}</span>
                  </div>
                  <div className="flex items-center gap-2">
                    <span className="w-16 shrink-0 text-xs text-neon-cyan">{r.resultRoiLabel}</span>
                    <div className="h-2.5 flex-1 overflow-hidden rounded-full bg-white/10">
                      <motion.div
                    ...
                    <span className="w-20 shrink-0 text-right text-xs font-semibold tabular-nums text-neon-cyan">
                      {calc.roi.toFixed(1)}×
                    </span>
                  </div>
                </div>
              ...
              <p className="mt-3 text-center text-xs leading-relaxed text-slate-500">{r.disclaimer}</p>
            </div>
          </div>
        </Reveal>
      </div>
