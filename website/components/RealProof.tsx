  ...
  return (
    <section id="proof" className="relative border-y border-white/5 bg-white/[0.015] py-24">
      ...
      <div className="mx-auto max-w-6xl px-5">
            ...
            return (
              <Reveal key={l.title.en} delay={i * 0.05}>
                <div className="flex h-full flex-col items-center rounded-2xl border border-white/10 bg-ink-900/50 p-4 text-center">
                  ...
                  <p className="mt-1 text-xs leading-relaxed text-slate-400">{l.desc[lang]}</p>
                </div>
              </Reveal>
            );
          })}
            ...
            <div className="grid grid-cols-2 gap-4 sm:grid-cols-3 lg:grid-cols-6">
              {p.metrics.map((m) => (
                <div key={m.label.en} className="text-center">
                  ...
                  <div className="mt-1 text-xs leading-tight text-slate-400">{m.label[lang]}</div>
                </div>
              ))}
            </div>
            <p className="mx-auto mt-5 max-w-3xl text-center text-xs text-slate-500">{p.metricsNote[lang]}</p>
          </div>
        </Reveal>
        ...
        <div className="grid gap-5 lg:grid-cols-3">
          ...
          <Reveal>
            <div className="flex h-full flex-col rounded-2xl border border-white/10 bg-ink-900/60 p-5">
              ...
              <p className="mt-auto pt-4 text-xs leading-relaxed text-slate-500">{p.mediaRealNote[lang]}</p>
            </div>
          </Reveal>
        ...
        {/* 官方频道最新案例（自动拉取，拿不到时优雅隐藏） */}
        <ChannelFeed />

        <p className="mx-auto mt-10 max-w-3xl text-center text-xs leading-relaxed text-slate-500">{p.disclaimer[lang]}</p>
      </div>
    </section>
  );
}
