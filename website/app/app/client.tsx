...
export default function MiniAppClient({ initialView }: { initialView: View }) {
  ...
  useEffect(() => {
    ...
    // startapp 深链格式 "<入口>__<campaign>"：前段路由视图，后段是帖子归因标识
    const [spEntry, spCampaign] = (startParam ?? "").split("__");
    ...
    try {
      ...
      if (!hasViewParam && spEntry) {
        const v = resolveView(spEntry);
        if (v !== initialView) {
          landed = v;
          setView(v, false);
        }
    ...
    if (!opened.current) {
      ...
      track("miniapp_open", {
        view: landed,
        source: spEntry || "direct",
        ...(spCampaign ? { cmp: spCampaign } : {}),
        platform,
      });
    }
  }, [initialView, setView]);
