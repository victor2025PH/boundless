...
interface Stats {
  ...
  ab?: Record<string, Record<string, { expose: number; click: number; ctr: number }>>;
  attribution?: { key: string; sessions: number; convert: number; rate: number }[];
  campaigns?: { key: string; summary: string; web: number; mini: number; sessions: number; convert: number; rate: number }[];
  growth?: { days: string[]; channel: (number | null)[]; group: (number | null)[] };
  wow?: {
...
const ATTR_LABELS: Record<string, string> = {
  direct: "直达 / 无来源",
  "telegram/organic": "Telegram（无参数跳转）",
};

// campaign 前缀 → 发布类型人话（daily-0707 这类日期后缀原样保留）。
function campaignLabel(key: string, summary: string): string {
  const base = summary
    ? summary
    : key === "overview"
      ? "置顶总览帖"
...
// A/B 实验 → 人话（实验 id 与各桶文案，便于后台直接读懂在测什么）。
const AB_LABELS: Record<string, { name: string; variants: Record<string, string> }> = {
  hero_cta: {
    name: "首页主 CTA 文案",
...
export default function AdminPage() {
  ...
  async function exportLeadsCsv() {
    ...
    const head = ["时间", "称呼", "联系方式", "意向", "来源", "获客渠道", "语言", "状态", "次数"];
    const esc = (v: string | number) => `"${String(v ?? "").replace(/"/g, '""')}"`;
    const lines = [head.map(esc).join(",")];
    for (const l of rows) {
      lines.push(
        [l.t, l.name, l.contact, l.interest, l.source, l.utm ?? "", l.lang, STATUS_META[l.status].label, l.count]
          .map(esc)
          .join(","),
      );
    }
            ...
            {tab === "growth" && (
              <div className="space-y-3">
                ...
                {stats.growth && stats.growth.days.length > 0 && (
                  <SectionCard title="频道 / 群订阅增长（每日快照）" Icon={UserPlus} accent="text-sky-300">
                    <div className="grid gap-3 sm:grid-cols-2">
                      {(
                        [
                          { label: "官方频道", data: stats.growth.channel, color: "#38bdf8" },
                          { label: "交流群", data: stats.growth.group, color: "#a78bfa" },
                        ] as const
                      ).map((m) => {
                        const nums = m.data.filter((v): v is number => v !== null);
                        const cur = nums[nums.length - 1] ?? 0;
                        const prev = nums.length > 1 ? nums[0] : cur;
                          ...
                          <div key={m.label} className="rounded-xl border border-slate-800 bg-slate-950/40 p-3">
                            <div className="flex items-center justify-between">
                              ...
                              <span className="text-[10px] text-slate-500">
                                {stats.growth!.days[0]?.slice(5)} ~ {stats.growth!.days[stats.growth!.days.length - 1]?.slice(5)}
                              </span>
                            </div>
                            <div className="mt-1 flex items-end gap-2">
                              <span className="text-2xl font-bold text-white">{cur}</span>
                              <span className={`pb-0.5 text-[11px] font-medium ${delta > 0 ? "text-emerald-400" : delta < 0 ? "text-rose-400" : "text-slate-500"}`}>
                                {delta > 0 ? `+${delta}` : delta}
                              </span>
                            </div>
                            {nums.length > 1 && <Sparkline data={nums} color={m.color} w={240} h={36} />}
                          </div>
                        );
                      })}
                    </div>
                ...
                {(stats.attribution?.length ?? 0) > 0 && (
                  <SectionCard title="获客归因（会话来源 → 留资转化）" Icon={TrendingUp} accent="text-cyan-300">
                    <ConvTable title="来源格式：source/medium/campaign；频道帖按钮点击自动携带" rows={stats.attribution ?? []} labels={ATTR_LABELS} />
                    <p className="mt-2 text-[10px] leading-relaxed text-slate-600">
                      telegram/channel/* 来自频道帖「官网」按钮；telegram/bot/* 来自机器人菜单；telegram/organic 是无参数的 TG 跳转；direct 为直达或无 referrer。
                    </p>
                  </SectionCard>
                )}

                {(stats.campaigns?.length ?? 0) > 0 && (
                  <SectionCard title="发布效果下钻（帖子/入口 → 会话与转化）" Icon={Megaphone} accent="text-amber-300">
                    <table className="w-full text-[11px]">
                      <thead>
                        <tr className="text-slate-500">
                          <th className="text-left font-medium">帖子 / 入口</th>
                          <th className="text-right font-medium">官网会话</th>
                          <th className="text-right font-medium">小程序会话</th>
                          <th className="text-right font-medium">转化</th>
                          <th className="text-right font-medium">转化率</th>
                        </tr>
                      </thead>
                      <tbody>
                        {(stats.campaigns ?? []).map((c) => (
                          <tr key={c.key} className="text-slate-300">
                            <td className="max-w-[260px] truncate py-0.5 pr-2 text-left" title={campaignLabel(c.key, c.summary)}>
                              {campaignLabel(c.key, c.summary)}
                            </td>
                            <td className="py-0.5 text-right">{c.web}</td>
                            <td className="py-0.5 text-right">{c.mini}</td>
                            <td className="py-0.5 text-right text-emerald-300">{c.convert}</td>
                            <td className="py-0.5 text-right font-medium text-cyan-300">{c.rate}%</td>
                          </tr>
                        ))}
                      </tbody>
                    </table>
                    <p className="mt-2 text-[10px] leading-relaxed text-slate-600">
                      每条频道帖的「官网/小程序」按钮各带唯一标识；此表回答「哪条帖子在带流量、带成交」。日更帖按 daily-月日 区分。
                    </p>
                  </SectionCard>
                )}

                {stats.ab && Object.keys(stats.ab).length > 0 && (
                  <SectionCard title="A/B 实验（曝光 → 点击 CTR）" Icon={Activity} accent="text-fuchsia-300">
                    <div className="space-y-4">
                      {Object.entries(stats.ab).map(([exp, variants]) => {
                        const meta = AB_LABELS[exp];
                        const entries = Object.entries(variants).sort(([a], [b]) => a.localeCompare(b));
                        const best = Math.max(...entries.map(([, v]) => v.ctr));
                        return (
                          <div key={exp}>
                            <div className="mb-2 text-xs font-medium text-slate-300">{meta?.name ?? exp}</div>
                            <div className="grid gap-2 sm:grid-cols-2">
                              {entries.map(([v, d]) => (
                                <div
                                  key={v}
                                  className={`rounded-xl border p-3 ${
                                    d.ctr === best && best > 0 ? "border-emerald-500/40 bg-emerald-500/5" : "border-slate-800 bg-slate-950/40"
                                  }`}
                                >
                                  <div className="text-[11px] text-slate-400">{meta?.variants[v] ?? `变体 ${v.toUpperCase()}`}</div>
                                  <div className="mt-1.5 flex items-end justify-between">
                                    <div className="text-lg font-bold text-white">{d.ctr}%</div>
                                    <div className="text-[10px] text-slate-500">
                                      曝光 {d.expose} · 点击 {d.click}
                                    </div>
                                  </div>
                                </div>
                              ))}
                      ...
                      <p className="text-[10px] leading-relaxed text-slate-600">
                        同一访客桶恒定（localStorage 分桶）；建议积累 ≥200 曝光/桶再下结论，赢家固化后开下一轮实验。
                      </p>
                    </div>
                  </SectionCard>
                )}
