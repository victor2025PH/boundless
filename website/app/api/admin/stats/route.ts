...
export async function GET(req: NextRequest) {
  ...
  const classifyRef = (ref: string): string => {
    ...
    try {
      ...
      if (/(^|\.)t\.me$|telegram/i.test(h)) return "telegram/organic";
      if (/google|bing|duckduckgo|yandex|baidu/i.test(h)) return `search/${h.split(".").slice(-2)[0]}`;
      return `referral/${h}`;
    } catch {
      return "direct";
    }
  ...
  const isExternalRef = (ref: string): boolean => {
    if (!ref) return false;
    try {
      return new URL(ref).hostname !== siteHost;
    } catch {
      return false;
    }
  };
  const attrSessions: Record<string, { utm: string; ref: string; lead: boolean }> = {};
  for (const e of events) {
    const sid = String(e.sid ?? "");
    if (!sid || String(e.event ?? "").startsWith("miniapp_")) continue;
    const s = (attrSessions[sid] ??= { utm: "", ref: "", lead: false });
    if (!s.utm && e.utm) s.utm = String(e.utm);
    if (!s.ref && isExternalRef(String(e.ref ?? ""))) s.ref = String(e.ref);
    if (e.event === "lead_submit") s.lead = true;
  }
  const attrAgg: Record<string, { sessions: number; convert: number }> = {};
  for (const s of Object.values(attrSessions)) {
    const key = s.utm || classifyRef(s.ref);
    (attrAgg[key] ??= { sessions: 0, convert: 0 });
    attrAgg[key].sessions++;
    if (s.lead) attrAgg[key].convert++;
  }
  const attribution = Object.entries(attrAgg)
    .map(([key, v]) => ({
      key,
      sessions: v.sessions,
      convert: v.convert,
      rate: v.sessions > 0 ? Number(((v.convert / v.sessions) * 100).toFixed(1)) : 0,
    }))
    .sort((a, b) => b.sessions - a.sessions)
    .slice(0, 20);

  // ── 发布效果下钻：按 utm_campaign（帖子/入口标识）聚合会话与转化 ──
  // 官网会话来自 utm 第三段；小程序会话来自 miniapp_open 的 cmp（startapp "__" 后缀）。
  // 一个 sid 只归一个 campaign（首个出现），转化 = 该会话内任一留资/解锁。
  const campBySid: Record<string, { cmp: string; mini: boolean; convert: boolean }> = {};
  for (const e of events) {
    const sid = String(e.sid ?? "");
    if (!sid) continue;
    const ev = String(e.event ?? "");
    const rec = (campBySid[sid] ??= { cmp: "", mini: false, convert: false });
    if (ev === "miniapp_open") {
      const c = propStr(e, "cmp");
      if (c && !rec.cmp) {
        rec.cmp = c;
        rec.mini = true;
      }
  ...
  const campAgg: Record<string, { web: number; mini: number; convert: number }> = {};
  for (const r of Object.values(campBySid)) {
    if (!r.cmp) continue;
    const a = (campAgg[r.cmp] ??= { web: 0, mini: 0, convert: 0 });
    if (r.mini) a.mini++;
    else a.web++;
    if (r.convert) a.convert++;
  }
  // campaign → 发布内容摘要（取该 campaign 最近一次发布的 summary）
  const allPubs = await listPublishes();
  const campSummary: Record<string, string> = {};
  for (const p of allPubs) {
    if (p.campaign && !campSummary[p.campaign]) campSummary[p.campaign] = p.summary;
  }
  const campaigns = Object.entries(campAgg)
    .map(([key, v]) => {
      const sessions = v.web + v.mini;
      return {
        key,
        summary: campSummary[key] ?? "",
        web: v.web,
        mini: v.mini,
        sessions,
        convert: v.convert,
        rate: sessions > 0 ? Number(((v.convert / sessions) * 100).toFixed(1)) : 0,
      };
    })
    .sort((a, b) => b.sessions - a.sessions)
    .slice(0, 30);
  ...
  // ── 频道/群订阅数曲线：growth.jsonl（每日 cron 快照），同日取最后一条 ──
  const growthRows = await readJsonl(path.join(ANALYTICS_DIR, "growth.jsonl"), 400);
  const growthByDay: Record<string, { channel: number | null; group: number | null }> = {};
  for (const g of growthRows) {
    const d = dayKey(g.t);
    if (d === "unknown") continue;
    growthByDay[d] = {
      channel: typeof g.channel === "number" ? g.channel : null,
      group: typeof g.group === "number" ? g.group : null,
    };
  }
  const growthDays = Object.keys(growthByDay).sort().slice(-30);
  const growth =
    growthDays.length > 0
      ? {
          days: growthDays,
          channel: growthDays.map((d) => growthByDay[d].channel),
          group: growthDays.map((d) => growthByDay[d].group),
        }
      : undefined;
