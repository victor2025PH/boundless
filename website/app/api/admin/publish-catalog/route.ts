...
export async function POST(req: NextRequest) {
  ...
  for (const p of posts) {
    const res = usePhoto
      ? await broadcastPhoto({ photo: p.imagePath, caption: p.text, target, withButton: true, campaign: `catalog-${p.id}` })
      : await broadcastMessage({ text: p.text, target, withButton: true, campaign: `catalog-${p.id}` });
  ...
  if (withOverview) {
    ...
    const res = await broadcastPhoto({ photo: ov.imagePath, caption: ov.caption, target, withButton: true, campaign: "overview" });
