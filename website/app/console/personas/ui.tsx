...
// 客户选择复用 ../ui 的 CustomerPicker（异步搜索，替代 500 客户预灌下拉）。

import { useState } from "react";
import { useRouter } from "next/navigation";
import { Flame, Link2, ShieldAlert, X } from "lucide-react";
import { CustomerPicker, type CustomerOption } from "../ui";
    ...
    <button
      ...
      className={`rounded-full border px-2.5 py-1 text-[11px] font-medium transition disabled:cursor-not-allowed disabled:opacity-60 ${
        item.granted
          ...
          : "border-slate-700 bg-ink-900/60 text-slate-500 hover:border-slate-500 hover:text-slate-300"
      }`}
    >
...
// ── 归属客户（与订单/授权行内控件同款「搜客户 / 快捷新建」模式）────
export function AssignPersonaCustomerControl({ personaId }: { personaId: string }) {
  const router = useRouter();
