# mobile-auto0423 — Codex 项目指令

> 本仓库 = **Facebook / Messenger 移动端自动化 bot**，双 worker 同 repo 协作。
> 本文件是 Codex 在本 cwd 下自动读取的项目级 memory，所有 Codex session（A/B）都会加载。
...
- **身份**：本 repo 双 worker 协作（A = greeting bot，B = messenger chat bot）；两 Codex 共用 `victor2025PH` token，身份由**物理机器 + `INTEGRATION_CONTRACT.md §二` 的独占区**决定，**不**由本文件自动 assign。进入本 repo 前先确认自己改的文件归谁。
...
- **共享 Codex 记忆**：`~/.Codex/projects/C--telegram-mtproto-ai/memory/` 里 `MEMORY.md` 按项目分组（同一 Codex 账号轮流在 A/B repo 工作，单点记忆库）。
...
4. 读 `~/.Codex/projects/C--telegram-mtproto-ai/memory/MEMORY.md` 的 "Project: mobile-auto0423" 段，重建上下文
5. （历史参考）`docs/runbook/B_RESUME_2026-04-23-EVENING.md` 是 B Codex 当时的恢复脚本

## sibling Codex 长任务前置检查（2026-04-26 加，PR #111/#112 事故教训）
...
事故根因：PR #142 期间 sibling Codex 用 `git commit -a` 把我 staged 的 5 个 P2 文件**打包进它的 OPT-FP1 messenger commit 6f80638**。AGENTS.md 文字警告"共享 worktree"靠人 review 不可靠，工具兜底:
...
**Codex 新 session / 崩溃后**：除了上面"崩溃恢复"5 步，还应跑 `repo_health.bat -Fetch` 看 sibling 期间发生了什么。
