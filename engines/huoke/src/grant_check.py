# -*- coding: utf-8 -*-
r"""huoke 人设授权软门控薄适配（PERSONA_BUS v1.2）。

包装 ``platform/identity/grant_gate.py``。本轮**不改**人设加载热路径——只交付检查器；
接入时在加载点加约 2 行即可。

接线指南（加载 fb_target / studio 人设调用点）::

    from grant_check import check_persona_grant
    r = check_persona_grant(source_key, product_id)  # product_id 通常 zhituo
    if not r["allowed"]:
...
拉取缓存（cron / 手动，建议挂在 deploy/cron export 之后）::
    ...
    python tools/persona_bus/fetch_grants.py --system huoke \
        --out engines/huoke/data/persona_grants_cache.json
...
def main(argv: list[str] | None = None) -> int:
    ...
    if len(argv) != 2:
        print(
            "usage: python src/grant_check.py <source_key> <product_id>\n"
