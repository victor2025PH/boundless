...
async function toggleRecording(){
  if(_recording) await vsStopRecording();
  else await vsStartRecording();
}

/* 2026-08-16 P3 撞名修复：原名 startRecording/stopRecording 与 system.js 录屏页
   同名函数冲突，system.js 后加载覆盖本组 → 投屏弹窗录制按钮实际调到录屏页逻辑
   （读不到 rec-device 直接 toast「请选择设备」），本组浏览器侧 MediaRecorder 录制
   从未生效。改前缀 vs（video-stream）。 */
