...
EXPECTED_PAGES = {
    ...
    "scheduled-jobs", "operator", "quick-actions", "sync-mirror", "op-replay",
...
def test_nav_sections_have_persist_keys():
    ...
    assert secs == {"leads", "acquire", "acquire-more", "devices", "auto", "data", "system"}
    for key in ("sec-leads", "sec-acquire", "sec-acquire-more", "sec-devices", "sec-auto", "sec-data", "sec-system"):
...
def test_boot_lang_apply_and_sw_cache_bumped():
    ...
    assert "reachx-v12-p3" in pwa_src  # SW 缓存版本随发布波次 bump（陈旧混合缓存实锤教训）
    assert "core.js?v=20260816p6" in DASHBOARD_HTML  # 资产版本（2026-08-16 P3 撞名清欠波次）
