...
def _resolve_dbs(args: argparse.Namespace) -> List[Tuple[str, Path, Dict[str, Any]]]:
    ...
    # 候选布局按「生产实际 → 历史/异机」排：本机双实例把库落在 <数据根>/config/
    # （与 config.yaml 同目录，因为进程 CWD 就是数据根、代码里写的是相对 config/）；
    # 另两种是早期单实例与直接指到库所在目录的布局。首个命中即用。
    for root in resolve_data_roots(args.data_root):
        db = None
        for cand in (Path(root) / "config" / "inbox.db",
                     Path(root) / "data" / "inbox.db",
                     Path(root) / "inbox.db"):
            if cand.is_file():
                db = cand
                break
