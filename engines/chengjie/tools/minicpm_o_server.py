...
class BaseEngine:
    ...
    def health(self) -> Dict[str, Any]:
        # engine 特征键:supervisor 用它确认「7861 上答话的真是自家 worker」。
        # 真机踩坑(2026-07-14):176 上无关的 sbv2_tts_server 绑 0.0.0.0:7861,
        # worker(127.0.0.1:7861)未就绪的窗口期 health/反代落到它身上,其 /health
        # 也回 model_loaded:true → supervisor 误判 ready、WS 反代被秒关。
        return {"engine": "minicpm-o", "model_loaded": bool(self.model_loaded),
...
def _patch_token2wav_profiler(model) -> None:
    """给 token2wav 的 stream() 包计时:量化「声码器 vs LLM」各占多少墙钟。

    RTF>1 时必须先知道慢在哪层再调参(n_timesteps 只影响声码器;若大头在 LLM
    出 token,调它无用)。日志每轮末由 _generate 读取汇总,零额外开销。
    """
    try:
        tok = model.tts.audio_tokenizer
        if getattr(tok, "_t2w_profiled", False):
            return
        orig = tok.stream

        def timed_stream(*a, **kw):
        ...
        tok.stream = timed_stream
        tok._t2w_profiled = True
        logger.info("token2wav profiler attached")
    except Exception:
        logger.warning("token2wav profiler attach 失败(仅影响观测)", exc_info=True)


_T2W_STATS = {"calls": 0, "secs": 0.0}


def _t2w_snapshot() -> tuple:
    return _T2W_STATS["calls"], _T2W_STATS["secs"]
...
class MiniCPMOEngine(BaseEngine):
    ...
    def load(self) -> None:
        ...
        with self._load_lock:
            ...
            try:
                ...
                # n_timesteps: token2wav flow-matching ODE 步数,默认 10。实测 RTF≈4
                # (生成慢于实时 4x → 通话「说1秒停3秒」)。降到 4 步给 token2wav 提速
                # (音质轻微降,通话可接受)。
                # enable_float16 默认关:stepaudio2 的 float16 是半成品——只 half 了 flow,
                # set_stream_cache→setup_cache 路径无 autocast,campplus 声纹 float32 进
                # spk_embed_affine_layer(half) 直接崩 "mat1 and mat2 must have the same
                # dtype"→ token2wav_cache=None → 整通零音频(2026-07-14 实测)。
                import os as _os
                _nts = int(_os.environ.get("MINICPM_N_TIMESTEPS", "4"))
                _fp16 = _os.environ.get("MINICPM_TOKEN2WAV_FP16", "0") == "1"
                model.init_tts(n_timesteps=_nts, enable_float16=_fp16)
                logger.info("init_tts n_timesteps=%d float16=%s", _nts, _fp16)
                _patch_token2wav_profiler(model)
                self.model = model
                self.model_loaded = True
    ...
    def _load_ref(self, ref_bytes: bytes):
        ...
        try:
            ...
            # token2wav 流式缓存 att_cache_buffer 上限 ~1000 帧(≈6.2s@16k):参考音过长
            # → init_token2wav_cache 崩(tensor size 1026 vs 1000)→ token2wav_cache=None
            # → streaming_generate 崩(NoneType subscript)→ 整通电话零音频。截到 4s:
            # 足够克隆音色(数秒即可),远低于上限,任何上传参考音都安全。(真机踩坑 03:22)
            max_samples = 16000 * 4
            if audio.size > max_samples:
                mid = audio.size // 2
                half = max_samples // 2
                audio = audio[max(0, mid - half): mid + half]   # 取中段(避开首尾静音/换气)
            return audio
        finally:
            try: os.unlink(rp)
...
class MiniCPMOSession(_QueueSession):
    ...
    def _init_voice(self) -> None:
        """session 级一次性初始化:token2wav 声学克隆 + system(含参考音)prefill 一次。

        参考音编码是重活(数秒音频过 encoder)。若放在每轮 _generate 里重复 prefill,
        TTFB 从 ~2s 涨到 ~18s、且多轮状态污染成复读(真机 03:48 实测回归)。官方
        streaming recipe 亦为 session 级一次性 prefill system → 参考音只编码这一次,
        轮间不再重塞;人设+音色靠保留的 KV 跨轮延续。
        """
        with self.engine._lock:
            try:
                m = self.engine.model
                m.reset_session(reset_token2wav_cache=True)
                # 音色克隆只靠 token2wav_cache 的参考音(声码器层决定音色)。**不**把参考音
                # 塞进 LLM system KV:那会让每个音频块都重复 attend 这 4s 音频 → RTF 从 ~1
                # 飙到 4(通话「说1秒停3秒」)。LLM 韵律/情感靠文字 system prompt 引导即可。
                if self.ref is not None:
                    m.init_token2wav_cache(prompt_speech_16k=self.ref)
                # 纯文本人设 session 级一次性 prefill(不含参考音 → KV 轻、RTF<1;
                # 只 prefill 一次 → 不重复污染多轮状态,_generate 每轮只喂用户音频)。
                _sp = self.system_prompt or "你是温暖真诚的陪伴者。"
                m.streaming_prefill(
                    session_id=self.sid,
                    msgs=[{"role": "system", "content": [_sp]}],
                    omni_mode=False, is_last_chunk=True)
            except Exception:
                logger.warning("init_voice 失败（音色克隆/人设可能退化）", exc_info=True)
    ...
    def _generate(self, pcm_bytes: bytes) -> None:
        ...
        try:
            ...
            with self.engine._lock:
                ...
                # 无参考音时 token2wav_cache=None,streaming_generate(generate_audio=True)
                # 会崩 NoneType subscript(--noref 对照/未配声纹的会话都会踩)→ 用无音色
                # 默认底噪 0.5s 静音当参考建缓存,保证「没配参考音也能出声」。
                if self.ref is None and getattr(m, "token2wav_cache", None) is None:
                    m.init_token2wav_cache(
                        prompt_speech_16k=np.zeros(16000 // 2, dtype=np.float32))
                # system(含参考音)已在 _init_voice 里 session 级 prefill 过一次,
                # 这里**只** prefill 用户音频 → 参考音不再逐轮重编码(TTFB ~2s 而非 ~18s)。
                step = 16000
                nchunks = max(1, (audio.size + step - 1) // step)
                for i in range(nchunks):
                    ch = audio[i * step:(i + 1) * step]
                    # 末块 <1s 必须 pad 到整秒:whisper encoder 对超短块报
                    # "Output size is too small"(官方 streaming_chat.py 同款处理;
                    # 真实通话 VAD 切出的音频长度天然不是整秒,首个真机拨测即踩中)。
                    if i == nchunks - 1 and ch.size < step:
                        ch = np.pad(ch, (0, step - ch.size))
                    m.streaming_prefill(
                ...
                # **不**在轮末 reset_session:那会清掉 _init_voice 的 system(人设+参考音)KV,
                # 令第 2 轮起丢人设/音色。保留 KV → system 延续 + 天然多轮记忆(更不机械)。
                # streaming_generate 结束时模型已自动置 new_user_msg=True,下一轮用户 prefill
                # 会正确开新轮。(真机 03:48 复读「你好你好」即旧「每轮重塞 system」污染所致)
            c1, s1 = _t2w_snapshot()
            wall = time.perf_counter() - t_turn0
            logger.info("[turn] wall=%.2fs token2wav=%.2fs(%d calls) llm+other=%.2fs",
                        wall, s1 - s0, c1 - c0, wall - (s1 - s0))
...
class WorkerSupervisor:
    ...
    def _worker_health(self, timeout: float = 1.0) -> Optional[Dict[str, Any]]:
        try:
            ...
            return None
        # 特征键校验:worker(127.0.0.1)未绑定的窗口期,同端口 0.0.0.0 的无关服务
        # (真机=sbv2_tts_server 绑 0.0.0.0:7861)会接走这条请求,其 /health 恰好也回
        # model_loaded:true → 误判就绪、WS 反代被秒关。非自家 engine 一律当未就绪。
        if h.get("engine") != "minicpm-o":
            return None
        return h
