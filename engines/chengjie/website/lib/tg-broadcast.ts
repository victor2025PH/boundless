 ...
 *  由 D:\workspace\brand-assets\build_brand_assets.py 生成（防高光穿孔管线），
 *  sync_brand_targets.py 分发至 public/brand/logos/。
 *  账号识别体系：频道/资源号 = 渐变光环版（官方资源识别符）；群/主号 = 纯主标版。 */
function brandAvatarPath(kind: "brand" | "ring" = "brand"): string {
  const file = kind === "ring" ? "boundless-avatar-ring.png" : "boundless-avatar.png";
  return path.join(process.cwd(), "public", "brand", "logos", file);
}
...
export async function setupChannels(opts?: {
  ...
  if (opts?.setPhoto !== false) {
    results.push(await setChatPhoto(token, `@${TELEGRAM_CHANNEL}`, brandAvatarPath("ring")));
    results.push(await setChatPhoto(token, `@${TELEGRAM_GROUP}`, brandAvatarPath("brand")));
  }
