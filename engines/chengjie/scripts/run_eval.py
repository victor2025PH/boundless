...
def main(argv=None) -> int:
    ...
    if args.media_consistency:
        from src.eval.media_consistency_eval import (
            evaluate_media_consistency, format_media_consistency_report,
        )
        report = evaluate_media_consistency()
        if args.json:
            print(json.dumps(report, ensure_ascii=False, indent=2))
        else:
            print(format_media_consistency_report(report))
        return 0 if report["passed"] else 1

    if args.bazi:
        from src.eval.bazi_chart_eval import (
            evaluate_bazi_chart, format_bazi_chart_report,
        )
        report = evaluate_bazi_chart()
        if args.json:
            print(json.dumps(report, ensure_ascii=False, indent=2))
        else:
            print(format_bazi_chart_report(report))
        if not report.get("available"):
            return 0  # 缺资源优雅跳过（与其他 eval 同约定）
        return 0 if report["passed"] else 1

    if args.bazi_reading:
        from src.eval.bazi_reading_eval import (
            evaluate_reading_quality, format_reading_report,
        )
        gen = _try_build_llm_generate_fn()
        if gen is None:
            print("[note] LLM 解读评测需 EVAL_LLM=1 且配好 ai（与其他 LLM 轨同约定）。"
                  "当前不可用，跳过。")
            return 0
        report = evaluate_reading_quality(gen)
        if args.json:
            print(json.dumps(report, ensure_ascii=False, indent=2))
        else:
            print(format_reading_report(report))
        if not report.get("available"):
            return 0
        return 0 if report["passed"] else 1
