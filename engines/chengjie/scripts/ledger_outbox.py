...
def _as_outbox_file(p: Path) -> Path:
    """目录形态 → 目录内标准文件名 ``ledger_outbox.jsonl``；其余原样返回。

    部署侧把 ``CHENGJIE_LEDGER_OUTBOX`` 当**落盘目录**注入（``<数据根>\\ledger_outbox``，
    且实例骨架预建该目录），而本模块原语义是**文件路径**——目录已存在时
    ``open(dir, 'a')`` 必败且 record_issue fail-silent，签发记录会无声丢失。
    故对已存在目录兼容为写目录内标准文件名（= run_export.ps1 license 段首选候选）；
    文件路径与尚不存在的路径维持原语义，向后兼容。"""
    return p / OUTBOX_FILENAME if p.is_dir() else p
...
def record_issue(record: dict, outbox_path=None) -> bool:
    ...
    try:
        if not isinstance(record, dict):
            return False
        path = _as_outbox_file(Path(outbox_path)) if outbox_path \
            else _default_outbox_path()
...
def export_outbox(input_path=None) -> dict:
    ...
    src = _as_outbox_file(Path(input_path)) if input_path else _default_outbox_path()
    by_key: dict = {}
    if not src.exists():
...
def _selftest() -> int:
    ...
    try:
        ...
        ob_dir = tmp / "outbox_dir"
        ob_dir.mkdir()
        os.environ[ENV_OUTBOX] = str(ob_dir)
        check("env 指向已存在目录 → 写 <目录>\\ledger_outbox.jsonl（部署形态）",
              record_issue(r3) is True and (ob_dir / OUTBOX_FILENAME).is_file())
        check("目录形态 export 同样解析到目录内文件",
              len(export_outbox()["records"]) == 1)
        os.environ.pop(ENV_OUTBOX, None)
        check("显式 outbox_path 传目录 → 同落标准文件名",
              record_issue(r2, ob_dir) is True and len(
                  (ob_dir / OUTBOX_FILENAME).read_text(
...
def main() -> int:
    ...
    if args.selftest:
        return _selftest()
    if args.export:
        src = (_as_outbox_file(Path(args.input)) if args.input
               else _default_outbox_path()).resolve()
