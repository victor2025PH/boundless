#!/usr/bin/env python3
# -*- coding: utf-8 -*-
r"""chengjie 人设授权软门控薄适配（PERSONA_BUS v1.2）。

包装 ``platform/identity/grant_gate.py``。本轮**不改**人设加载热路径——只交付检查器；
接入时在加载点加约 2 行即可。

接线指南（加载 profiles_runtime / 绑定人设调用点）::

    from grant_check import check_persona_grant
    r = check_persona_grant(source_key, product_id)  # product_id 如 zhiliao/tongyi
    if not r["allowed"]:
...
默认缓存路径（可用 env PERSONA_GRANT_CACHE 覆盖）::
    ...
    # 双实例：建议各实例 AITR_DATA_DIR 下各一份，或统一 PERSONA_GRANT_CACHE

拉取缓存（cron / 手动，建议挂在 deploy/cron export 之后）::

    python tools/persona_bus/fetch_grants.py --system chengjie \
        --out engines/chengjie/data/persona_grants_cache.json
...
def main(argv: list[str] | None = None) -> int:
    ...
    if len(argv) != 2:
        print(
            "usage: python scripts/grant_check.py <source_key> <product_id>\n"
