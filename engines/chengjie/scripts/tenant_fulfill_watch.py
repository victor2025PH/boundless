...
def run_once(
    *,
    ...
    for order, args in todo:
        ...
        # ── 可达性闸门：expose 失败 / 无 URL / 公网探测不过 → 持单（绝不交付死链）──
        reachable = bool(ok2 and public_url and public_url.startswith("http")
                         and probe(public_url))
        if not reachable:
            held += 1
            ...
            continue
        code = tf.build_delivery_code(public_url, token, args["instance_id"], username=user)
        if writeback_fn(oid, code):
            ...
            if persist:
                save_state(st)
            handled += 1
            # 到期账本：从**交付成功**起算（持单期是我方公网问题，不吃客户时长）
            if card_update_fn is not None:
                days = tf.period_days(order)
                card_update_fn(args["instance_id"], {
            ...
            print(f"[开通] {oid} ✓ 已回填交付信息，website 私信客户\n        {code}")
            results.append({"order_id": oid, "action": "activated",
                            "public_url": public_url, "instance_id": args["instance_id"]})
        else:
...
def _self_test() -> int:
    ...
    print("[self-test] PASS ✓ 装机忽略/托管全链/转人工/幂等/令牌不外泄/持单台账↔送达摘除/"
          "到期账本随交付起算/生产state零写入" if ok else "[self-test] 有 FAIL")
