  ...
  var MANIFEST =
{
  ...
  "cards": [
    ...
    { "id": "origin", "tab": "customer", "surfaces": ["web"],
      "keyWeb": "inbox.cp.h.origin",
      "note": "2026-08-18 跨平台档案（cp-origin → /api/workspace/origin*）：客户从哪个平台来/原平台称呼/聊过的话题域，坐席补录后服务端渲染 _origin_block 注入 AI（contacts.origin_profile.enabled 热闸，默认关）。P0 仅 web；app 端待右栏灰度读数后再收敛" },
    ...
    { "id": "xlate", "tab": "tools", "surfaces": ["web", "app"],
      ...
      "note": "2026-08-17 新增：单次翻译四件套（图片/语音/多线路对照/文档）共享组件 cp-xlate-tools，直连收件箱同一批 translate-* 端点；收件箱「对话翻译」弹层底部的四按钮保留为第二入口（同端点单源逻辑，未来按用量退役）。弱会话依赖：无会话也可用（no-ctx 豁免），auto 目标语仅在有会话时走服务端解析。2026-08-17 调序：置 tools 首位（网页端「通用工具」组在前的重组布局；app 端 no-ctx 空态下它是唯一可用卡，前置同样更优）" },
    { "id": "convops", "tab": "tools", "surfaces": ["app"],
      "keyWeb": "inbox.convops.title", "keyApp": "cp.app.h_convops", "converged": true,
      "note": "会话运维。app 已落地 v1（P1-4 2026-08-12，cp-conv-ops.js：档位状态+暂停全自动+搁置1h/4h+归档两步确认）。2026-09-06 #206（M-4 D，D-M7）web 卡整块撤下：标签/归档/删除与会话行右键·⋯菜单完全重复、两个删除语义不明、加标签下拉被卡体裁切、折叠区空壳（skuio 确认）——web 侧去处：行菜单（标签编辑早已浮层）+「全自动运行中」状态卡迁 AI 状态弹层顶部 + 认领按钮回会话头部；app 侧不动（壳右栏无行菜单，仍需要它）" },
    ...
    { "id": "image", "tab": "tools", "surfaces": ["web", "app"],
      "keyWeb": "inbox.cp.h.image", "keyApp": "cp.app.h_image", "converged": true,
      "note": "2026-08-21 工具箱新增：坐席 AI 生成图片（cp-image → /api/image/*，复用 SelfieProvider+image_gate+comfy_infer 多引擎 flux/qwen_edit/z_image）。弱会话依赖（无会话可生成/存册/下载，发送需会话；App 端有 no-ctx 豁免）。2026-08-22 App 收敛；桌面壳无桥（stub）→ App boot 脚本运行时 hidden，桥接线后揭示" },
    { "id": "nurture", "tab": "tools", "surfaces": ["web", "app"],
      "keyWeb": "inbox.cp.h.nurture", "keyApp": "cp.app.h_nurture", "converged": true,
      "note": "2026-08-21 工具箱新增：智能养号（cp-nurture → /api/nurture/*，状态复用 fleet-health，用户自配每号养护方案写 ops.nurture overlay；引擎 dry_run→canary go_live + 动作探针）。弱会话依赖（App 端有 no-ctx 豁免）。2026-08-22 App 收敛；桌面壳无桥（stub）→ App boot 脚本运行时 hidden，桥接线后揭示。2026-08-22 可懂化改版（实施55）：术语人话化（试点账号/模拟运行/单步试跑，术语表钉 cp-i18n 养号分区注释）+ 三步引导条 + go_live 内联确认 + 影子记录可读化 + 探针/警告收高级折叠 + can_write 只读分层（后端字段随重启窗，前端 fail-open）；行为门禁 tools/verify_nurture_ui.py（-Full，55 断言）" },
