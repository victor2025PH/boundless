...
// 同一形态事实也给安装器（installer.nsh `!include /NONFATAL flavor.nsh`）：非 clean 形态
// 的向导带「内测版」角标 + 品牌栏后缀，人手拿到包一眼分清，不再靠文件名/目录考古。
// 缺文件时 installer.nsh 按 internal 处理（角标常亮）——宁可内测包多个角标，不可
// 内测包冒充干净包出门（这正是角标存在的理由）。gitignore 同 build-info.json。
const flavorNsh = path.join(__dirname, "flavor.nsh");
fs.writeFileSync(
  flavorNsh,
  "; generated by build/write-build-info.js -- do not edit, do not commit\r\n" +
    '!define CX_FLAVOR "' + info.flavor + '"\r\n',
  "ascii"
);
console.log("[build-info] ok: " + flavorNsh + " (flavor=" + info.flavor + ")");
