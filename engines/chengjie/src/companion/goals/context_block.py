...
_DISCIPLINE = (
    ...
    "铺垫只能用真实聊过的内容，严禁编造共同经历/虚构过去当由头。"
)

# 限时档纪律（P1 2026-08-30）：与 natural 的差异是刻意的——「敷衍→彻底放下」
# 对用户显式拍板的冲刺太软（敷衍可换角度再试一次）；「明确拒绝/情绪低落→放下」
# 与越界红线原样保留（那是安全语义不是节奏语义）。
# 禁虚构铺垫（#110 实施91，0831 原图 880 实锤）：agenda 的「下个月…」推进项
# 被拿编造的共同用餐当铺垫完成将来邀约——目标压力 + 零共同记忆 = 自信编造，
# 两档纪律同钉（出站另有 strip_ungrounded_shared_past 兜底）。
_DISCIPLINE_SPRINT = (
