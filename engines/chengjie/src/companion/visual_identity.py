...
def identity_note(label: str, *, matched: str = "", confirmed: bool = False,
    ...
    lab = str(label or "")
    if lab == "persona":
        # 2026-09-18 生产首验（zhiliao 本地小模型 chatx）：旧措辞「不要当成对方发来的自拍去评价」
        # 仍被写成「这红唇配深蓝，绝了！」——小模型只认「该说什么」，不认「别怎么说」。
        # 改成正向指令 + 一句范例 + 明确禁项（把「夸对方外貌/穿搭」点名）。
        return ("图中人物是**你自己**——这是你自己的照片（与你的人设相貌一致，多半是我方此前发过、"
                f"现在被{who}发了回来）。按「对方把我的照片发回来了」接话——比如「哈哈你还存着我这张呢」"
                "「怎么突然翻出我这张」；**禁止**夸图中人的长相、妆容或穿搭，禁止说「你这拍得」「你这张」"
                "之类把照片当成对方本人的话。")
