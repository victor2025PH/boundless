    ...
    "ecommerce": {
        ...
        "entries": [
            ...
            {"title": "物流多久到货", "category": "订单查询",
             "triggers": ["多久到", "物流", "发货", "快递", "几天", "发货要多久"],
             "example_reply": "正常下单后 24 小时内发货，国内一般 2-4 天送达，偏远地区稍久。您把订单号发我，我帮您查实时进度。"},
            {"title": "怎么查物流", "category": "订单查询",
             "triggers": ["查物流", "物流单号", "查快递", "订单到哪了",
                          "track my order", "track order", "tracking", "where is my order"],
             "example_reply": "您把订单号发我，我帮您查询实时物流进度；发货后系统也会推送运单号，可在物流官网自助跟踪。/ Send me your order number and I'll track it for you — a tracking link is also sent after shipping."},
            {"title": "支持哪些支付方式", "category": "常规咨询",
             "triggers": ["怎么付款", "支付方式", "能用什么付"],
             "example_reply": "我们支持主流的在线支付方式，下单时按页面提示选择即可。如遇支付失败可换一种方式重试，或把截图发我帮您排查。"},
            {"title": "支持货到付款吗", "category": "常规咨询",
             "triggers": ["货到付款", "到付", "先发货后付款", "cash on delivery", "COD"],
             "example_reply": "部分地区支持货到付款，下单时若结算页有「货到付款」选项即可选择；不支持的地区建议使用在线支付。把您的收货地址发我，我帮您确认。"},
            {"title": "如何退换货", "category": "退款投诉",
             "triggers": ["退货", "换货", "退款", "不想要了", "怎么退货", "可以换货吗",
                          "return policy", "how to return", "refund policy", "exchange"],
             "example_reply": "支持 7 天无理由退换（影响二次销售除外）。请把订单号和原因发我，我为您发起售后流程并跟进。/ We offer 7-day hassle-free returns — send me your order number and I'll start the process."},
            {"title": "退款多久到账", "category": "退款投诉",
             "triggers": ["退款多久", "退款到账", "钱什么时候退", "退款进度", "refund time"],
             "example_reply": "退货签收后 1-3 个工作日内原路退回，银行处理一般再需 1-5 个工作日。把订单号发我，我帮您查退款进度。"},
            {"title": "运费多少", "category": "常规咨询",
             "triggers": ["运费", "邮费", "包邮", "运费多少钱", "shipping fee", "shipping cost"],
             "example_reply": "多数商品满额包邮，具体运费以结算页显示为准（按地区与重量计算）。把您的收货地区发我，我帮您估一下。"},
            {"title": "尺码怎么选", "category": "常规咨询",
             "triggers": ["尺码", "尺寸", "选码", "码数", "大小合适吗", "size guide"],
             "example_reply": "商品详情页有尺码对照表，按身高体重对照选择即可；两码之间建议选大一码。把您的身高体重发我，我帮您推荐。"},
            {"title": "可以开发票吗", "category": "常规咨询",
             "triggers": ["发票", "开发票", "开票", "报销", "invoice"],
             "example_reply": "可以的，支持电子发票。下单时在备注填写抬头与税号，或下单后把订单号和开票信息发我，我帮您登记。"},
