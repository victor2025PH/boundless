...
# 采集优先级（靠前先问）：生日 > 称呼 > 城市。城市另有 overseas-like 闸
# （``user_clock.city_ask_eligible``），本表只定「一次开场只问一个」的顺序。
PROFILE_SLOTS: Tuple[str, ...] = ("birthday", "name", "city")

# 各槽位的「主动问」directive（只学口吻、随口一问、不强求）。
# P0 2026-07-29：去掉「好久没聊了」硬前缀——沉默几小时也会触发，真实间隔由
# prompt 框定层按 silent_hours 分档表述，指令层不再自带时间感措辞。
_DIRECTIVES = {
    "birthday": (
        "主动开场：先自然轻松地问候一句；再像朋友间随口好奇那样，"
        "顺势问一句还不知道TA生日是哪天呢——别像填表或查户口，问完顺其自然，"
        "TA不想说也别追。"),
    "name": (
        "主动开场：先自然问候一句；再顺势轻松地问一句平时喜欢别人怎么称呼TA、"
        "想让我怎么叫TA——随口一问、别太正式，TA不想说也别勉强。"),
    "city": (
        "主动开场：先自然轻松地问候一句；再像朋友随口关心那样，"
        "问一句TA现在住哪个城市——别像查户口，别要详细地址，"
        "只是想对上作息和天气，TA不想说也别追。"),
}
