...
def resolve_cold_start_cfg(config: Optional[Mapping[str, Any]]) -> Dict[str, Any]:
    ...
    if "warmup_review" in cs:
        out["warmup_review"] = bool(cs.get("warmup_review"))
    return out
...
def automation_ceiling(
    connected_at: float, now: float, cfg: Mapping[str, Any]
) -> Optional[str]:
    """预热期内的**入站自动回复**档位上限：预热中 → ``"review"``，否则 ``None``（不封顶）。

    这是事故闭环的另一半。上面那些判据管「系统主动去骚扰谁」，本函数管「有人真的来
    消息时，AI 能不能不经人看就直接回」。.198 现场真正被对方识破、真正把额度烧穿的
    是后者：新号一登录，目录同步灌进来的历史关系里任何人开口，都被全自动 AI 接管，
    而那些人认识的是号主本人、不是 AI 人设（实录回话："你成Ai了吗"）。头几天让人看
    着发，不是关掉能力——``review`` 档 AI 照常拟稿，只是必须人点一下。

    **失败方向与 ``account_warming`` 刻意相反**（这是本函数存在的全部理由，别"顺手统一"）：
    - 那边 ``connected_at<=0``（未知）→ 保守判「预热中」＝**不发**。主动外呼漏一轮零代价。
    - 这边未知 → 返回 ``None``＝**不封顶**。因为「未知」在存量部署里是常态（账号注册表
      早于本功能、没有该行），若也保守封顶，所有老客户升级后自动回复会被静默降级成人审
      队列——没人盯队列时那就是一次真实的服务中断。两边代价函数不同，故失败方向不同。
    """
    if not cfg.get("enabled", True) or not cfg.get("warmup_review", True):
        return None
    if connected_at <= 0:
        return None
...
__all__ = [
    ...
    "resolve_cold_start_cfg", "account_warming", "automation_ceiling",
    "imported_no_inbound",
