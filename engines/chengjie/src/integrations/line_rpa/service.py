...
class LineRpaService:
    ...
    async def _maybe_run_auto_accept(self) -> None:
        ...
        try:
            ...
            if res.get("tapped"):
                logger.info(
                    "auto_accept_friends: tapped=%d coords=%s welcome=%s",
                    res["tapped"], res.get("coords"), res.get("welcome_enqueued"),
                )
            if res.get("welcome_enqueued"):
                try:
                    self._trigger_evt.set()
                except Exception:
                    pass
                try:
                    wn = int(res.get("welcome_enqueued") or 0)
                    if wn > 0:
                        sq_w = await self._runner.run_send_queue_deliveries(
                            max_deliver=max(1, wn))
                        if sq_w.get("delivered") or sq_w.get("failed"):
                            self._last_run["friend_welcome_deliver"] = sq_w
                            logger.info(
                                "friend_welcome_deliver delivered=%s failed=%s",
                                sq_w.get("delivered"), sq_w.get("failed"),
                            )
                except Exception:
                    logger.debug("friend_welcome_deliver 失败", exc_info=True)
        except Exception:
