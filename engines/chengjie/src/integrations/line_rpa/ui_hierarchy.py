...
_ACCEPT_TEXTS = frozenset(
    {"accept", "同意", "承認", "추가", "허락", "add", "agree", "confirm"})
_ACCEPT_RID = frozenset({"accept", "agree", "confirm"})


@dataclass
class FriendAcceptRow:
    """好友申请行：显示名 + 接受按钮坐标。"""
    name: str
    accept_x: int
    accept_y: int
...
def _is_accept_label(text: str, cdesc: str, rid: str) -> bool:
    t = (text or "").strip().lower()
    d = (cdesc or "").strip().lower()
    r = (rid or "").strip().lower()
    return (
        any(k in t for k in _ACCEPT_TEXTS)
        or any(k in d for k in _ACCEPT_TEXTS)
        or any(k in r for k in _ACCEPT_RID)
    )


def find_friend_accept_rows(xml_bytes: bytes) -> List[FriendAcceptRow]:
    """从好友申请页 XML 提取「显示名 + 接受按钮」行（自上而下）。

    启发式：接受按钮同行左侧最近的非按钮 TextView 视为 display_name。
    """
    try:
        root = ET.fromstring(xml_bytes)
    except ET.ParseError:
    ...
    accept_nodes: List[Tuple[int, int, int, int, int, int]] = []
    name_nodes: List[Tuple[str, int, int, int, int]] = []
    ...
    for el in root.iter():
        ...
        if r - l < 20 or b - t < 12:
            continue
        cx, cy = (l + r) // 2, (t + b) // 2
        if _is_accept_label(text, cdesc, rid):
            accept_nodes.append((cx, cy, l, t, r, b))
            continue
        ...
        if _is_accept_label(text, cdesc, rid):
            continue
        low = text.lower()
        if any(k in low for k in _SKIP_TEXT_SUBSTR):
            continue
        if low in {"ok", "cancel", "拒绝", "decline", "block"}:
            continue
        name_nodes.append((text, cx, cy, l, r))

    rows: List[FriendAcceptRow] = []
    for ax, ay, al, at, ar, ab in accept_nodes:
        best_name = ""
        best_score = -1
        for name, nx, ny, nl, nr in name_nodes:
            if nr >= al - 8:
                continue
            if abs(ny - ay) > max(48, (ab - at) * 1.5):
                continue
            score = (al - nr) + max(0, 40 - abs(ny - ay))
            if score > best_score:
                best_score = score
                best_name = name
        if best_name:
            rows.append(FriendAcceptRow(name=best_name, accept_x=ax, accept_y=ay))

    rows.sort(key=lambda x: x.accept_y)
    return rows
