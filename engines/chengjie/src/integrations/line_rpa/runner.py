...
_CHAT_VISION_PROMPT = (
    ...
    "   - 左侧视频缩略图（画面中央有 ▶ 播放按钮或角落有时长标注；算 video）\n"
    "   - 左侧动图（角落有 GIF 角标的循环动画；算 gif）\n"
    ...
    '{"role":"peer|self|none","kind":"text|image|video|gif|sticker|voice|file|other",'
    '"content":"...","desc":"..."}\n'
    "字段规则：\n"
    "- role=peer → 最底部对方消息；role=self → 最底部是己方；role=none → 找不到\n"
    "- kind=text：content 填原文（保留全部标点和 emoji），desc 空串\n"
    "- kind=sticker：content 空串，desc 用 ≤20 中文字描述贴图形象和动作（如 '棕熊欢呼撒彩纸'）\n"
    "- kind=image：content 空串，desc 用 ≤80 中文字描述图片可见内容（主体/场景/可读文字要点，"
    "如 '键盘快捷键列表截图，标题写着常用组合键'）；看不清的部分不要编造\n"
    "- kind=video：content 空串，desc 用 ≤40 中文字描述缩略图画面 + 时长（如 '海边日落缩略图，0:15'）；"
    "只描述缩略图可见内容，不要假装看过整段视频\n"
    "- kind=gif：content 空串，desc 用 ≤40 中文字描述动图动作/表情梗（如 '小猫疯狂点头'）\n"
...
class LineRpaRunner:
    ...
    async def maybe_auto_accept_friends(self, max_accept: int = 5) -> Dict[str, Any]:
        ...
        - Phase16：可选 ``auto_accept.welcome`` —— 接受后入队 companion 问候
        """
        ...
        welcome_cfg = parse_welcome_cfg(aa_cfg)
        welcome_on = bool(welcome_cfg.get("enabled"))
        welcome_lang = (
            str(welcome_cfg.get("lang") or "").strip()
            or str(self._cfg_get("default_reply_lang") or "zh"))

        out: Dict[str, Any] = {
            "tapped": 0, "coords": [], "welcome_enqueued": 0,
        }
        try:
            for _ in range(max(1, int(max_accept or 1))):
                xml_bytes, _ = await asyncio.to_thread(self._dump_ui_xml)
                if not xml_bytes:
                    break
                rows = ui.find_friend_accept_rows(xml_bytes)
                if not rows:
                    if out["tapped"] == 0:
                        out["skipped"] = True
                        out["reason"] = "no_accept_buttons"
                    break
                row = rows[0]
                adb.input_tap(serial, row.accept_x, row.accept_y)
                await asyncio.sleep(0.6)
                out["tapped"] += 1
                out["coords"].append([row.accept_x, row.accept_y])
                logger.info(
                    "auto_accept_friends: tapped (%d,%d) name=%r",
                    row.accept_x, row.accept_y, row.name)

                if welcome_on and row.name.strip():
                    text = build_welcome_text(lang=welcome_lang, welcome_cfg=welcome_cfg)
                    qid = enqueue_friend_welcome(
                        self._state_store, peer_name=row.name.strip(), text=text)
                    if qid:
                        out["welcome_enqueued"] += 1
                        logger.info(
                            "friend_welcome enqueued peer=%r qid=%s",
                            row.name, qid)
            ...
            if out["tapped"] and self._state_store is not None:
                try:
                    self._state_store.insert_alert(
                        ...
                        detail={
                            "coords": out["coords"],
                            "welcome_enqueued": out.get("welcome_enqueued", 0),
                        },
