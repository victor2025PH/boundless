...
class DeferredDispatcher:
    ...
    def __init__(
        ...
        *,
        ...
        activity_provider: Optional[Callable[[str, str, str], float]] = None,
        recent_activity_guard_sec: float = 0.0,
    ) -> None:
        self._store = store
        ...
        # 主动消息（reactivation）发送前活跃复核：defer 窗口内（调度→真发可隔 15min-4h）
        # 对方若已开口，权威事实源（收件箱 last_ts）会更新 → 取消这条过时的「好久不见」。
        # None/0 = 不启用（旧行为）。仅对 proactive 行生效（普通回复不受影响）。
        self._activity_provider = activity_provider
        self._recent_guard_sec = max(0.0, float(recent_activity_guard_sec))
    ...
    async def _dispatch_one(self, row: Dict[str, Any], now: float) -> bool:
        ...
        # 期间对方若已开口，权威事实源（收件箱 last_ts）会更新 → 取消这条已过时的「好久不见」，
        # 而非发出（修「刚聊过还发主动问候」的 defer 竞态）。仅对 proactive 行生效。
        if self._activity_provider is not None and self._recent_guard_sec > 0:
            extra = row.get("extra") if isinstance(row.get("extra"), dict) else {}
            reason = str(row.get("reason") or "")
            is_proactive = bool(extra.get("reactivation")) or reason.startswith("reactivation")
            if is_proactive:
                try:
                    last_ts = float(
                        self._activity_provider(platform, account_id, chat_key) or 0.0)
                except Exception:
                    last_ts = 0.0
                if last_ts > 0 and (now - last_ts) < self._recent_guard_sec:
                    self._store.mark_expired(row_id, "recent_activity")
                    logger.info(
                        "[deferred_outbox] 主动消息取消（对方近期活跃 idle=%.1fh < %.1fh）"
                        "id=%d platform=%s chat=%s",
                        (now - last_ts) / 3600.0, self._recent_guard_sec / 3600.0,
                        row_id, platform, chat_key)
                    try:
                        from src.monitoring.metrics_store import get_metrics_store
                        get_metrics_store().record_reactivation_skipped("recent_activity_drain")
                    except Exception:
                        pass
                    return False
