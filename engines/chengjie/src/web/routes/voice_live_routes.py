...
def register_voice_live_routes(app, *, api_auth=None, config_manager=None) -> None:
    ...
    @app.get("/api/voice/call/readiness")
    async def voice_call_readiness(request: Request):  # noqa: ANN001
        """原生通话开闸前就绪度体检：配置 + 主机探测 + 参考音 + auto_ai 会话 → blocker/warning。

        「看→校→开」范式的原生通话版：ops 卡在零来电时也能看清「差哪些」。纯函数
        ``evaluate_call_readiness`` 是单一事实源（与 watchdog 告警同口径）；探针复用
        ``voicecall.health.probe_call_host``（60s TTL）；telegram_calls 未启用即 enabled=False。
        """
        from src.voicecall.health import evaluate_call_readiness, probe_call_host
        cfg = _full_cfg()
        host_probe = await asyncio.to_thread(probe_call_host, cfg)
        ref_summary = collect_voice_ref_readiness_summary()
        auto_ai = None
        try:
            store = getattr(request.app.state, "inbox_store", None)
            if store is not None:
                auto_ai = sum(1 for v in store.all_automation_modes().values()
                              if v == "auto_ai")
        except Exception:
            auto_ai = None
        res = evaluate_call_readiness(
            cfg, host_probe=host_probe, ref_summary=ref_summary,
