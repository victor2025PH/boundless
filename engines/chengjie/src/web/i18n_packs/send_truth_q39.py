...
ZH = {
    ...
    "inbox.cs.xlate_hold.low_conf": "译文质量不过关 · 这条没发 · {hhmm}",
    "inbox.cs.xlate_hold.low_conf_t": "译文看起来没翻对（目标语文字缺失 / 原样回吐 / 长度离谱），换引擎重译也没过关，按「不发原文」纪律扣住。点「重试翻译」再试，或手动改稿发；小语种（印地/孟加拉/泰米尔等）建议在翻译设置里指定更强的引擎",
...
EN = {
    ...
    "inbox.cs.xlate_hold.low_conf": "Translation quality too low · not sent · {hhmm}",
    "inbox.cs.xlate_hold.low_conf_t": "The translation looks wrong (target script missing / echoed source / abnormal length) and a second engine didn't pass either, so it was held under the “never send the original” rule. Click “Retry translation” or edit and send manually; for Indic and other low-resource languages, pin a stronger engine in translation settings",
...
ZH_HANT = {
    ...
    "inbox.cs.xlate_hold.low_conf": "譯文品質不過關 · 這條沒發 · {hhmm}",
    "inbox.cs.xlate_hold.low_conf_t": "譯文看起來沒翻對（目標語文字缺失 / 原樣回吐 / 長度離譜），換引擎重譯也沒過關，按「不發原文」紀律扣住。點「重試翻譯」再試，或手動改稿發；小語種（印地 / 孟加拉 / 泰米爾等）建議在翻譯設定裡指定更強的引擎",
