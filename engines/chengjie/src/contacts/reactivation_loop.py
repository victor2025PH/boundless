...
# last_activity_provider：(channel, account_id, chat_key) -> 该会话最近一次互动的
# epoch 秒（0=未知）。权威事实源（统一收件箱 conversations.last_ts），用于校正
# journey.updated_at 陈旧导致的「刚聊过还发好久不见」——真实近期活跃则不主动打扰。
LastActivityProvider = Callable[[str, str, str], float]
...
- **不要用「好久不见 / 好久没联系 / 久しぶり / long time no see」这类"隔了很久"的开场**——
  直接从上面那件具体事自然续上即可（是否真的隔很久由系统判断，你不要假设）
- 不要主动索要联系方式 / 不发链接
...
class ReactivationLoop:
    ...
    def __init__(
        ...
        *,
        ...
        last_activity_provider: Optional[LastActivityProvider] = None,
        min_silent_sec: float = 3 * 86400.0,
        ai_name: str = "她",
        max_per_tick: int = 3,
        ...
        # 真实近期活跃校验（权威事实源=统一收件箱）：journey.updated_at 常因跨平台/
        # 收件箱链路未回写而陈旧，导致「刚聊过几小时的人」仍被判为沉默 → 发「好久不见」。
        # 注入后：主动发送前用真实 last_ts 复核，近期活跃则跳过（并以真实值修正 silent_days）。
        self._last_activity_provider = last_activity_provider
        self._min_silent_sec = max(0.0, float(min_silent_sec))
    ...
    async def _schedule_one(self, cand: ReactivationCandidate) -> bool:
        ...
        # 单靠它选出的「沉默」候选可能其实刚聊过。真实活跃在 min_silent 窗口内 → 跳过；
        # 否则用真实 last_ts 修正 silent_days，让 prompt 的「X 天前」贴合事实。
        eff_silent_days = cand.silent_days
        if self._last_activity_provider is not None:
            try:
                last_ts = float(
                    self._last_activity_provider(channel, account_id, chat_name) or 0.0)
            except Exception:
                last_ts = 0.0
            if last_ts > 0:
                idle = time.time() - last_ts
                if idle < self._min_silent_sec:
                    logger.info(
                        "[reactivation] skip contact=%s 真实近期活跃 idle=%.1fh "
                        "(< min_silent=%.1fh) — 不发「好久不见」",
                        cand.contact_id, idle / 3600.0,
                        self._min_silent_sec / 3600.0,
                    )
                    try:
                        from src.monitoring.metrics_store import get_metrics_store
                        get_metrics_store().record_reactivation_skipped("recent_activity")
                    except Exception:
                        pass
                    return False
                # 真实沉默时长（比 journey.updated_at 更准）覆盖 silent_days
                eff_silent_days = round(idle / 86400.0, 2)
