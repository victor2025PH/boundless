...
def _provider_from_tag(tag: str) -> str:
    ...
    委托 vision_client._backend_from_tag 统一归因语义（按级联 tag **末段**判终态后端，
    修「ollama_empty_no_zhipu_key 因含 zhipu 字样被误归 zhipu」）；无法归因回落 "vision"
    （保 ocr namespace 旧键名，不产生 "none" 新键）。
    """
    try:
        from src.vision_client import _backend_from_tag
        backend = _backend_from_tag(tag)
        return backend if backend != "none" else "vision"
    ...
    async def translate_image(
        ...
        *,
        ...
        if cached_text is not None:
            ...
            if tag == "cache_hit":
                # 下层 vision 描述缓存命中（本层 ocr: 缓存被淘汰但下层还在）——
                # 未真调 provider，按缓存命中记，别污染后端成功率/延迟。
