...
def build_avatar_voice_profile(
    *, reference_audio_path: str, speaker_id: str,
    ...
    无云端音色 ID：合成时 TTSPipeline(backend=avatar_clone) 直接拿 reference_audio
    去本机 7852 零样本克隆（不烧任何云端配额）。reference_text（参考音逐字稿）
    填了音色相似度更高——enroll 路由会在缺省时用 AvatarHub STT 自动转写生成。
    enabled+owner_consent+reference_audio_path 齐备 → /api/voice/profiles 标 ready。
    """
    vp: Dict[str, Any] = {
        "enabled": True,
        ...
        "voice": "",
        "reference_audio_path": reference_audio_path,
        "emotion_default": str(emotion_default or "gentle"),
    }
    if reference_text:
        vp["reference_text"] = reference_text
    return vp
