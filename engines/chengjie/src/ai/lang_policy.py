...
#
# #143 C-补（2026-09-02 工单「切中文」真机制，HXP9YD 日志 13347-13349）：VLM 按
# prompt「简洁分条」多行落库——``[图片内容] 1. 可见物体：…\n\n2. 无票据/订单/证件/表单/
# 聊天记录``。旧正则只剥带标记的首行，续行「2. 无票据…」以裸中文漏进证据链 → 英文
# 会话判成 zh 强证据 → pin/expected=zh 把英文原稿改成中文。写入侧已改压单行；这里兼容
# **存量多行**：标记行之后的续行——编号行（``1.``/``2)``）、项目符号行（``-``/``•``）、
# 纯中文模板句（含汉字且无任何字母文字：拉丁/西里尔/泰/阿拉伯/假名/谚文…）——一并
# 剥除，空行不打断续行块。客户合并进来的外语正文带字母 → 不匹配 → 保留；标记**前**的
# caption 从未被碰（客户自己写的话仍是证据）。
_ALPHA_SCRIPT_CHARS = (
    r"A-Za-z\u00C0-\u024F\u0370-\u03FF\u0400-\u04FF\u0590-\u05FF\u0600-\u06FF"
    r"\u0900-\u097F\u0E00-\u0E7F\u3040-\u30FF\uAC00-\uD7AF"
)
