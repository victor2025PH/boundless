...
def build_colloquial_prompt(emotion: str = "neutral", lead: bool = True,
    ...
    if lvl == "vivid":
        ...
        parts = [
            ...
            # 2026-09-18：原文点名「说真的」「我跟你讲」→ 实测该会话几乎条条「说真的」
            # （人设 style/quirks/openers 三处又同时在教）。改成「不加」为默认、加也不许
            # 用这两个：口头禅一旦被推荐就会被 LLM 当模板照抄。
            "- 不要加「说真的」「我跟你讲」「讲真」「老实说」这类口头禅框架——原文没有就不加；"
            "主观口吻靠语序和语气词体现，不靠固定短语；",
            f"- 语气{tone}；",
