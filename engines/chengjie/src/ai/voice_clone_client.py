...
class VoiceCloneClient:
    ...
    def __init__(self, cfg: Optional[Dict[str, Any]] = None) -> None:
        ...
        # 集群鉴权头（AvatarHub 系服务如 .176 Fish/RVC/SBV2 要求 ``X-AH-Svc: <token>``）。
        # 令牌**绝不写进 config.yaml**（入 git）：config 只填 ``svc_token_env``＝密钥名，
        # 运行时从 env / .env.local / config/secrets.local.json 读取（与 voice_enroll 同源）。
        # ``svc_token`` 直填仅供测试。健康探测 /health 不带此头（契约：health 免鉴权）。
