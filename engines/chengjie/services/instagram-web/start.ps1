# start.ps1 — 启动 Instagram 个人号网页托管登录微服务（Playwright）
# 用法: pwsh -File start.ps1   (在 services/instagram-web 下)
# 说明: 为主进程提供服务器内官方登录（账密/2FA）+ 多账号保活；入站消息回推统一收件箱。
#       主进程需开 config.platform_login.instagram.web_enabled=true 并指向 web_url。
# ⚠️ 非官方接入（依赖 instagram.com DOM）：请用小号 + 独立代理，风险自负。

$ErrorActionPreference = "Stop"
Set-Location -LiteralPath $PSScriptRoot
$root = (Resolve-Path (Join-Path $PSScriptRoot "..\..")).Path

# 服务监听端口（须与主进程 platform_login.instagram.web_url 一致）
$env:PORT = "8793"
...
if ($env:PY_API_TOKEN) {
  Write-Host ("[instagram-web] auth_token resolved from " + $cfgOverlay + " / " + $cfgMain)
} else {
  Write-Host "[instagram-web] WARN: auth_token not found — inbound pushes may be 401-rejected"
}
# 交互登录默认 headed（弹窗内人工完成官方登录）；restore/恢复默认 headless。
$env:IG_HEADLESS = "0"
$env:IG_RESTORE_HEADLESS = "1"
$env:LOG_LEVEL = "info"

Write-Host "[instagram-web] starting on :$($env:PORT) (ingest=$($env:PY_INGEST_URL))"
$logDir = Join-Path $root "services\instagram-web\logs"
New-Item -ItemType Directory -Force -Path $logDir | Out-Null
$log = Join-Path $logDir ("instagram-web-" + (Get-Date -Format "yyyyMMdd") + ".log")
...
Add-Content -LiteralPath $log -Value ("[instagram-web] " + (Get-Date -Format o) + " launching node=" + ($(if ($nodeExe) { $nodeExe } else { "<NOT FOUND>" })) + " server=" + $serverJs)
if (-not $nodeExe) { Add-Content -LiteralPath $log -Value "[instagram-web] FATAL: node.exe not found on PATH nor common install dirs"; exit 1 }
