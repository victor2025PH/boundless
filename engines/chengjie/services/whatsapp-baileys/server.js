...
async function pushWaMessage(entry, msg, skipEmpty) {
  ...
  if (!fromMe && msg.key && msg.key.id) {
    if (!entry._lastInboundKey) entry._lastInboundKey = {};
    entry._lastInboundKey[jid] = msg.key;
  }
...
// 拟人已读回执：把该会话对端最近一条消息标记为已读（对端看到蓝色双勾/「已读」）。
// readMessages 需完整 key（含群 participant），故读缓存的 _lastInboundKey；无缓存→无未读
// 可标（marked:false）。best-effort：账号未连接 404，其余失败 500，Python 侧一律软降级。
app.post("/accounts/:id/read", async (req, res) => {
  const entry = findByAccount(req.params.id);
  if (!entry || !entry.sock) {
    return res.status(404).json({ ok: false, error: "account not connected" });
  }
  const jid = toJid((req.body && req.body.jid) || "");
  if (!jid) return res.status(400).json({ ok: false, error: "jid required" });
  const key = entry._lastInboundKey && entry._lastInboundKey[jid];
  if (!key) return res.json({ ok: true, marked: false });
  try {
    await entry.sock.readMessages([key]);
    res.json({ ok: true, marked: true });
  } catch (e) {
    logger.debug({ e }, "readMessages failed");
    res.status(500).json({ ok: false, error: String(e) });
  }
});

// 拟人打字/录音状态：向对端会话发 presence（composing=正在输入 / recording=正在录音 /
// paused=停止）。回复前的拟人节奏（对端看到「正在输入…」再收到消息）。best-effort。
const _WA_PRESENCE = new Set(["composing", "recording", "paused", "available", "unavailable"]);
app.post("/accounts/:id/typing", async (req, res) => {
  const entry = findByAccount(req.params.id);
  if (!entry || !entry.sock) {
    return res.status(404).json({ ok: false, error: "account not connected" });
  }
  const jid = toJid((req.body && req.body.jid) || "");
  if (!jid) return res.status(400).json({ ok: false, error: "jid required" });
  let state = String((req.body && req.body.state) || "composing");
  if (!_WA_PRESENCE.has(state)) state = "composing";
  try {
    await entry.sock.sendPresenceUpdate(state, jid);
    res.json({ ok: true, state });
  } catch (e) {
    logger.debug({ e }, "sendPresenceUpdate failed");
    res.status(500).json({ ok: false, error: String(e) });
  }
});
