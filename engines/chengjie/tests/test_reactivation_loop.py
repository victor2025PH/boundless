...
# ── 真实近期活跃校验（修「刚聊过几小时却发好久不见」）──────────────

@pytest.mark.asyncio
async def test_skips_when_recently_active(base_setup):
    """last_activity_provider 报告 2 小时前刚聊过（< min_silent 3天）→ 跳过，不发、不调 LLM。"""
    two_hours_ago = time.time() - 2 * 3600
    loop = _make_loop(
        base_setup,
        last_activity_provider=lambda ch, acc, ck: two_hours_ago,
        min_silent_sec=3 * 86400.0,
    )
    n = await loop.run_once()
    assert n == 0
    base_setup["send_callback"].assert_not_awaited()
    base_setup["ai_client"].chat.assert_not_awaited()  # 近期活跃直接短路，连 LLM 都不调


@pytest.mark.asyncio
async def test_sends_when_truly_silent(base_setup):
    """last_activity 报告 10 天前（> min_silent）→ 正常主动，并用真实沉默天数喂 prompt。"""
    ten_days_ago = time.time() - 10 * 86400
    loop = _make_loop(
        base_setup,
        last_activity_provider=lambda ch, acc, ck: ten_days_ago,
        min_silent_sec=3 * 86400.0,
    )
    n = await loop.run_once()
    assert n == 1
    base_setup["ai_client"].chat.assert_awaited()
    prompt = base_setup["ai_client"].chat.await_args.args[0]
    assert "10 天前" in prompt   # silent_days 用真实活跃修正（非陈旧 journey.updated_at）


@pytest.mark.asyncio
async def test_unknown_last_activity_does_not_block(base_setup):
    """last_activity=0（未知，如无收件箱记录）→ 不拦，退回旧行为（照常发）。"""
    loop = _make_loop(
        base_setup,
        last_activity_provider=lambda ch, acc, ck: 0.0,
        min_silent_sec=3 * 86400.0,
    )
    n = await loop.run_once()
    assert n == 1
    base_setup["send_callback"].assert_awaited()


@pytest.mark.asyncio
async def test_no_provider_keeps_legacy_behavior(base_setup):
    """未注入 last_activity_provider → 完全旧行为（用 candidate.silent_days）。"""
    loop = _make_loop(base_setup)  # 无 provider
    n = await loop.run_once()
    assert n == 1
    prompt = base_setup["ai_client"].chat.await_args.args[0]
    assert "5 天前" in prompt   # 用 candidate 的 silent_days=5.0
