...
async def test_tts_blocked_and_metered(tmp_path, monkeypatch):
    ...
    async def _fake_uncached(text, *, voice=None, timeout_sec=30.0, spec=None, **kw):
        # **kw：对 _synthesize_uncached 新增的关键字参数（如 colloquial_lead）保持宽容，
        # 本测试只关心额度拦截/记账，不关心合成参数细节。
