...
def test_global_auto_ai_default_without_persisted_mode(monkeypatch):
    """无显式档位但全局 auto_draft.automation_mode=auto_ai → 仍让位 System Z。"""
    cid = "whatsapp:PNID:wa:user:123"
    pb.register_inbox_store_getter(lambda: _FakeStore({}))  # 无持久化档位

    class _OrchWithCfg(_FakeOrch):
        _config = {"inbox": {"auto_draft": {"automation_mode": "auto_ai"}}}

    monkeypatch.setattr(
        "src.integrations.account_orchestrator.get_orchestrator",
        lambda *a, **k: _OrchWithCfg([("whatsapp", "PNID")]),
    )
    assert inbox_will_autosend("whatsapp", "PNID", "wa:user:123") is True
