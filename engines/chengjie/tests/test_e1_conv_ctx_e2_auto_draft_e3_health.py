...
class TestE2AutoGenerateDraft:
    ...
    def test_media_placeholder_never_counts_as_same_message(self, tmp_store):
        """占位符相等不算同一条消息（防语音会话永久死锁）。

        2026-08-22 WA 实锤：转录在 enrich 之后且只回写消息行，草稿快照永远停在
        「[语音]」→ 下一条语音进来文本也是「[语音]」→ 判重跳过 → 转录不跑 →
        再下一条仍是占位符，会话再也收不到回复。
        """
        svc = self._make_svc(tmp_store)
        conv = {"conversation_id": "wa:default:voice", "platform": "whatsapp",
                "account_id": "default", "chat_key": "voice", "display_name": "V"}
        old_id = tmp_store.upsert_draft({
            "source_kind": "inbox",
            "conversation_id": "wa:default:voice",
            "platform": "whatsapp",
            "peer_text": "[语音]",
            "draft_text": "旧回复",
            "risk_level": "low",
        ...
        result = svc.auto_generate_draft(conv, "[语音]")
        assert result is not None and result != old_id
        assert tmp_store.get_draft(old_id)["status"] == "cancelled"
