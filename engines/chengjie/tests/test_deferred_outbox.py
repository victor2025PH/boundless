...
async def test_proactive_cancelled_when_recently_active():
    """reactivation 行在 defer 窗口内对方已活跃（idle < guard）→ 发送前取消（expired），不发。"""
    s = DeferredOutboxStore(":memory:")
    s.enqueue(platform="telegram", account_id="a", chat_key="123",
              reply_text="久しぶり！", defer_until=NOW - 10, now=NOW - 100,
              reason="reactivation:silent_5d", extra={"reactivation": True})
    sent, send = _recorder()
    # 对方 2h 前刚活跃；guard=3天 → 取消
    d = _disp(s, {"telegram": send},
              activity_provider=lambda p, a, c: NOW - 2 * 3600,
              recent_activity_guard_sec=3 * 86400.0)
    n = await d.run_once(now=NOW)
    assert n == 0 and sent == []
    assert s.count(status="expired") == 1


async def test_proactive_sent_when_truly_silent():
    """reactivation 行且对方确实久未活跃（idle > guard）→ 正常发出。"""
    s = DeferredOutboxStore(":memory:")
    s.enqueue(platform="telegram", account_id="a", chat_key="123",
              reply_text="嘿最近怎么样", defer_until=NOW - 10, now=NOW - 100,
              reason="reactivation:silent_5d", extra={"reactivation": True})
    sent, send = _recorder()
    d = _disp(s, {"telegram": send},
              activity_provider=lambda p, a, c: NOW - 10 * 86400,  # 10 天前
              recent_activity_guard_sec=3 * 86400.0)
    n = await d.run_once(now=NOW)
    assert n == 1 and len(sent) == 1


async def test_non_proactive_not_affected_by_activity_guard():
    """普通（非 reactivation）消息不受活跃复核影响——即便对方刚活跃也照常发。"""
    s = DeferredOutboxStore(":memory:")
    s.enqueue(platform="telegram", account_id="a", chat_key="123",
              reply_text="这是普通回复", defer_until=NOW - 10, now=NOW - 100,
              reason="care:xxx")   # 非 reactivation
    sent, send = _recorder()
    d = _disp(s, {"telegram": send},
              activity_provider=lambda p, a, c: NOW - 60,  # 1 分钟前刚活跃
              recent_activity_guard_sec=3 * 86400.0)
    n = await d.run_once(now=NOW)
    assert n == 1 and len(sent) == 1   # 非主动消息不拦
