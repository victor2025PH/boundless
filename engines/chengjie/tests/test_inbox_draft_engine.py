...
@pytest.mark.asyncio
async def test_generate_inbox_draft_media_hint_when_photo_requested(tmp_path):
    """发图协同 hint（文图一致性第一防线）：对方在要照片且 selfie 开 →
    草稿 LLM 收到「本文本只是图片失败时的兜底、禁承诺」指令；生成后清扫不残留。"""
    _reset_metrics()
    cm = await _make_cm(
        tmp_path,
        {"enabled": True, "db_path": str(tmp_path / "mh.db"),
         "vector": {"enabled": False}, "extract": {"enabled": False}},
    )
    cm.config["companion"] = {"selfie": {"enabled": True}}
    seen = {}

    async def _gen(**kw):
        seen["hint"] = str(
            (kw["user_context"] or {}).get("_media_coherence_hint") or "")
        return "嘿嘿想看我呀～"
    ...
    sm = SkillManager(cm, ai)
    out = await sm.generate_inbox_draft(
        text="發個照片給我看看嘛", chat_key="u8", platform="telegram",
        history=[{"role": "user", "content": "發個照片給我看看嘛"}],
    )
    assert out is not None
    assert "照片" in seen["hint"] and "不要" in seen["hint"]  # hint 已注入
    # finally 清扫：不残留到下一轮
    uc = sm._get_user_context("u8")
    assert "_media_coherence_hint" not in uc
    snap = _ms.get_metrics_store().get_inbox_draft_metrics()
    assert snap["total"].get("media_hint") == 1


@pytest.mark.asyncio
async def test_generate_inbox_draft_media_hint_offer_accept(tmp_path):
    """offer-accept 桥（draft 路径）：上一轮 AI 提议发照片、本条短肯定 → 同样注入 hint。"""
    _reset_metrics()
    cm = await _make_cm(
        tmp_path,
        {"enabled": True, "db_path": str(tmp_path / "mh2.db"),
         "vector": {"enabled": False}, "extract": {"enabled": False}},
    )
    cm.config["companion"] = {"selfie": {"enabled": True}}
    seen = {}
    ...
    sm = SkillManager(cm, ai)
    out = await sm.generate_inbox_draft(
        text="好呀", chat_key="u8b", platform="telegram",
        history=[
            {"role": "user", "content": "你长什么样呀"},
            {"role": "assistant", "content": "要不要我拍一张给你看呀？"},
            {"role": "user", "content": "好呀"},
        ],
    )
    assert out is not None
    assert "照片" in seen["hint"]


@pytest.mark.asyncio
async def test_generate_inbox_draft_no_media_hint_for_plain_chat(tmp_path):
    """普通闲聊/未开 selfie → 不注入 hint（零噪声）。"""
    _reset_metrics()
    cm = await _make_cm(
        tmp_path,
        {"enabled": True, "db_path": str(tmp_path / "mh3.db"),
         "vector": {"enabled": False}, "extract": {"enabled": False}},
    )
    cm.config["companion"] = {"selfie": {"enabled": True}}
    seen = {}
    ...
    sm = SkillManager(cm, ai)
    out = await sm.generate_inbox_draft(
        text="今天好想你", chat_key="u8c", platform="telegram",
        history=[{"role": "user", "content": "今天好想你"}],
    )
    assert out is not None
    assert seen["hint"] == ""
