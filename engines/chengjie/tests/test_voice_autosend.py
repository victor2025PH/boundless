...
class _FakeResult:
    def __init__(self, ok, audio_path, provider="avatar_clone", duration_sec=1.5):
        self.ok = ok
...
class _FakeTTS:
    ...
    async def synthesize(self, text, timeout_sec=45.0, emotion=None, **kwargs):
        _FakeTTS.last_text = text
        _FakeTTS.last_kwargs = dict(kwargs)
...
def test_voice_metrics_record_and_snapshot():
    before = va.metrics_snapshot()
    va.record_voice_sent(1234, synth_meta={
        "provider": "avatar_clone", "fallback_from": "avatar_clone",
        "synth_text_len": 12, "persona_id": "lin_xiaoyu",
        "truncation_suspect": True, "truncation_retried": True,
    })
    ...
    assert after["last_provider"] == "avatar_clone"
    assert after["last_fallback_from"] == "avatar_clone"
    assert after["last_synth_text_len"] == 12
    assert after["last_persona_id"] == "lin_xiaoyu"
    assert after["truncation_suspects"] == before.get("truncation_suspects", 0) + 1
    assert after["truncation_retries"] == before.get("truncation_retries", 0) + 1
