...
def test_routes_registered():
    ...
    assert "/api/voice/call/readiness" in paths    # 原生通话开闸前就绪度体检


def _call_app(telegram_calls: dict):
    from fastapi import FastAPI
    from src.web.routes.voice_live_routes import register_voice_live_routes
    app = FastAPI()
    cm = types.SimpleNamespace(config={
        "realtime_voice": {"enabled": True, "base_url": "http://h:7860"},
        "telegram_calls": telegram_calls})
    register_voice_live_routes(app, api_auth=None, config_manager=cm)
    return app


def test_call_readiness_disabled():
    from fastapi.testclient import TestClient
    j = TestClient(_call_app({"enabled": False})).get("/api/voice/call/readiness").json()
    assert j["ok"] is True and j["enabled"] is False and j["ready"] is False


def test_call_readiness_host_unreachable_blocks(monkeypatch):
    from fastapi.testclient import TestClient
    import src.voicecall.health as H
    monkeypatch.setattr(H, "probe_call_host",
                        lambda cfg, **kw: {"reachable": False, "model_loaded": False})
    monkeypatch.setattr("src.web.routes.voice_live_routes.collect_voice_ref_readiness_summary",
                        lambda *a, **k: {"persona_count": 0, "with_reference": 0, "worst_grade": "none"})
    j = TestClient(_call_app({"enabled": True, "brain": "s2s"})).get(
        "/api/voice/call/readiness").json()
    assert j["ok"] is True and j["enabled"] is True and j["ready"] is False
    assert any("主机不可达" in b for b in j["blockers"])


def test_call_readiness_all_clear(monkeypatch):
    from fastapi.testclient import TestClient
    import src.voicecall.health as H
    monkeypatch.setattr(H, "probe_call_host",
    ...
    monkeypatch.setattr("src.web.routes.voice_live_routes.collect_voice_ref_readiness_summary",
                        lambda *a, **k: {"persona_count": 0, "with_reference": 0, "worst_grade": "none"})
    # transport_verified: 运营跑过 tg_call_poc 三闸门后置 true，否则传输层恒 blocker
    j = TestClient(_call_app({"enabled": True, "brain": "s2s",
                              "transport_verified": True})).get(
        "/api/voice/call/readiness").json()
    assert j["ready"] is True and j["blockers"] == []
