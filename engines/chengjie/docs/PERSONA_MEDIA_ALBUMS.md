...
启用开关（2026-07-31 起**两级**，SSOT＝`src/companion/photo_capability.py`）：

1. **全局总闸** `companion.selfie.enabled`（与形象照同口径；关＝两条链都不查相册）；
2. **每人设开关** `persona.capabilities.photos`（**默认关**）——人设工作室「相册」页
   顶部「相册 / 发图功能」toggle。人设级关闭＝该人设整条发图能力关闭（注册相册 +
   AI 生成 + [PHOTO] 指令 + 承诺异步兑现 + 主动生活照），且对话 prompt 注入
   「不承诺发照片」硬约束（与 `capabilities.video_call` 同款反向消费）、出站守卫
   剥「我找找/翻相册/这张是…」类空头承诺（人设试聊 2026-07-31 实录事故收口）。
   两级都开才真发图；试聊抽屉每条回复的 meta 行显示「相册：开/关」。
