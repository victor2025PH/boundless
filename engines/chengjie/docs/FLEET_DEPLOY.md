...
- 节点 node_key 只在 `%ProgramData%\ChatX\fleet\agent.json`。父目录 `%ProgramData%\ChatX` 先设受保护 DACL：SYSTEM（S-1-5-18）和 Administrators（S-1-5-32-544）完全控制，Users（S-1-5-32-545）只有读和执行，设完用同一套父目录判断做后置校验。父目录已经是这个状态时不再 `/reset`。父目录和 `fleet` 都不能是 junction / 重解析点。`fleet` 在第一次 `/reset` 之前采样「已锁定」：已经锁定就跳过 reset 和 grant（含一切 `/T`），服务每次 `save` 不再把目录解锁。还没锁定时才对目录本身 `/setowner`、`/reset`（不带 `/T`）、`/inheritance:r /grant:r`（只含 SYSTEM 和 Administrators），然后做后置校验。校验通过后、任何 `/T` 之前，不跟随重解析点地枚举全部子项，发现 reparse 就中止。通过之后才看子文件：所有者不是 SYSTEM 或 Administrators 的删掉，删除失败就中止。最后才 `/setowner /T`、对子项 `/reset /T`、再 `/grant /T`，然后再校验一次。读取时目录没锁，或者文件所有者不可信，都不采用这份文件。`/grant:r` 单独用不会清掉其他 SID 的显式 ACE。从旧版（目录还没锁）升级时，锁好之后重写 `agent.json`：只保留 `node_id`、`node_key`、`heartbeat_sec`、`enroll_secret`、`pending_request_id`、`pairing_code`；`controller_url` 用安装包的 `-Controller`（或安装时的控制器地址）重写；`instances` 清空（里面的 `restart_cmd`、`base_url`、`config_path`、`auth_token` 一并丢掉）。升级后要重新探测 instances，`restart_cmd` 要管理员重新配置。旧版默认安装下本地用户能读到 `node_key`，建议升级后在主控侧轮换 `node_key`。`restart_cmd` 由 SYSTEM 用 `shell=True` 执行，所以不能信一份来历不明的 `agent.json`。主控只存哈希。待批准申请另有一把安装时生成的 `enroll_secret`，主控只存它的 sha256；轮询必须带上它。配对码可以给人看，不是凭证。
