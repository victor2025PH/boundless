...
- 节点 node_key 只在 `%ProgramData%\ChatX\fleet\agent.json`。父目录 `%ProgramData%\ChatX` 先设受保护 DACL：SYSTEM（S-1-5-18）和 Administrators（S-1-5-32-544）完全控制，Users（S-1-5-32-545）只有读和执行，设完做后置校验。父目录已经是这个状态时不再 `/reset`。父目录给 Users 只读之后，ChatX 目录下的其他组件也不能再往这里写，它们需要自己的可写目录。父目录不能是 junction。`fleet` 已经锁定就原样跳过，不再跑 icacls。`fleet` 不存在、没锁、或者本身是重解析点时，不对旧路径做任何 icacls：先把 `agent.json` 快照到管理员的 TEMP，再把旧目录改名为 `fleet.legacy-<guid>`（改名不跟随链接，旧目录保留），然后在已锁的父目录里新建空的 `fleet`，只做本层 setowner 和受保护 DACL（SYSTEM 与 Administrators 完全控制）。建好后校验已锁定，并且确认它不是重解析点。没有 `/T`。身份只从这份快照迁入：只保留 `node_id`、`node_key`、`heartbeat_sec`、`enroll_secret`、`pending_request_id`、`pairing_code`；`controller_url` 用安装包的 `-Controller`（或安装时的控制器地址）重写；`instances` 清空（里面的 `restart_cmd`、`base_url`、`config_path`、`auth_token` 一并丢掉）。升级后会留下 `fleet.legacy-<guid>`，确认新节点正常后可以手动删除。升级后要重新探测 instances，`restart_cmd` 要管理员重新配置。旧版默认安装下本地用户能读到 `node_key`，建议升级后在主控侧轮换 `node_key`。读取时目录没锁，或者文件所有者不可信，都不采用这份文件。`restart_cmd` 由 SYSTEM 用 `shell=True` 执行，所以不能信一份来历不明的 `agent.json`。主控只存哈希。待批准申请另有一把安装时生成的 `enroll_secret`，主控只存它的 sha256；轮询必须带上它。配对码可以给人看，不是凭证。
