...
|---|---|---|---|---|---|---|---|---|
| **J-1** | AI 出站内容正确性 + 记忆一致性 | #171 #175 #176 #177 | $190 | 已完成（代码侧） | A `7a3fc6c9` / B `9d6426f3` / C `24229010` / D `5d46c4ee`（模块+prompt 消费；**send 路由一行接线交 J-3**，J-3 活跃） | 交 J-2：SOP 步 prompt 人设口吻钉子；交 J-3：`unified_inbox_send_routes.py` L999 后一行 `record_human_outbound`；值守：skuio 机跑 `xlate_memory_purge --apply`、mutual_chat 阈值复核 | `发版对账_v1.0.74_J1.md` | ~$120 |
...
| **J-3** | LINE 媒体链 | #169 #172 #180 | $190 | 进行中 | A #172 定层+修复 `c9d19638`（Letter Sealing 媒体 em* 路径+解密；404≠过期；回填排程；探针扩四类） | B #180 / C #169 / D 观测 | `发版对账_v1.0.74_J3.md` | ~$60 |
