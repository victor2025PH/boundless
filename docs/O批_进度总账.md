...
|---|---|---|---|---|---|---|---|---|
| **O-1** | 拟人与反识破 | #252 #253 #254 #255 | $200 | **收工**（09-08 13:5x；A/B/C/D/E 全段落地，`.py` 需重启 → 装载 / 回归 / 打包 / 发布归 R78） | **A** `ddc5d282`（stop_contact / self_harm 硬停「最多一条」+ 冻结切人工 + risk=high 转人工审 + 词表；新 `stop_contact.py`）/ **B** `583af62b`（出站去 AI 标点·句式 + `[outbound]` 日志 + `scripts/outbound_style_gate.py` 200 样例 0 违规）/ **C** `ea20cefd`（陪伴域私人聊天底稿 `system_companion.txt` + `persona_guard.rewrite_service_tone` 三处接线，50/50 代理盲评）/ **D** `8c1012ac`（「拟人程度」三档 UI + 白名单 + 门禁 ①② 转绿）`5d51348f`（humanize 读·想·打字三档模型 + `[pacing]` 每条一行 + composing 六态 + 首回延迟 60–300s 留 pending + inbound_merge 8–15s 随机窗；`test_pacing_profiles` 28 例，20 条回放 r=0.93 无两条到秒相同）`2ddbb3ec`（出厂基线 A 类：profile=natural / inbound_merge / work_schedule 08:20–01:00 + min 种子 + 功能矩阵重导出）/ **E** `1081244e`（账号名≠人设名：`account_name_check` + status / profile 路由 `name_mismatch` + personas.html 红条 + TG/WA 一键按人设改名）`9a87d56b`→`63893c92`（>72h 沉寂首回剥编造迟回理由 → 「刚看到」，enrich_draft + 协议链；重放：9a87d56b 被 QQ 线 7383d15e 经共享索引反转）；docs `dba59c33 ea8831b9 54a8fb8b f1c7eadc b2ff74f2 81167a89` | 无；交下批：首回延迟 UI 字段 / GreetingSkill（skill_manager HOT-ZONE）接迟回守卫 / profile_push 是否升 A 类 / 关怀 peer_filter 加 frozen 过滤 / deferred 原文识别根治；**待老板知会 skuio**：夜间班表默认开（01–08 不回）；真机 composing=sent 实证交 R78 grep | `发版对账_v1.0.78_O1.md`（§二 A–E 全、§三验收、§四交下批 + 共享索引误卷登记；回访两句已改 `r78_252.txt`、changelog 中英各两句已进 `_notes_1078*.txt`） | ≈$35（A）+ ≈$45（D/E 续做；B/C 上一对话未报） |
...
- **15:3x 更新**：O-1 13:5x 已收工，上一条解除。老板拍板 **D-P3（修正）：1.0.78 不等 P 批，照发**——P 批（`docs/修复指令_P批_并行_2026-09-08.md`，1.0.77 复测的登录群发 / 停联丢失 / 谎称发图 / 出厂态）目标 1.0.79。R78 回访口径加一句：「1.0.78 发出去的消息零破折号；草稿预览里的净化、登录 / 切档不群发、发图不说谎随 1.0.79；现在别按账号开全自动」。
