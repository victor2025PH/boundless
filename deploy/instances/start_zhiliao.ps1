# start_zhiliao.ps1 — 智聊 ChatX 实例启动（chengjie 双实例，同目录 README.md 是运行手册）
# 用法: powershell -ExecutionPolicy Bypass -File .\start_zhiliao.ps1 [-DataDir <数据根>]
#
...
} else { Join-Path $PSScriptRoot 'zhiliao\data' }
...
if (-not (Test-Path $DataRoot)) {
    Fail "实例数据根不存在: $DataRoot`n  → 先按 deploy\instances\README.md §3.2 初始化（本脚本绝不自动创建，防止覆盖现网数据）"
}
$cfg = Join-Path $DataRoot 'config\config.yaml'
if (-not (Test-Path $cfg)) {
    Fail "实例主配置不存在: $cfg`n  → 按 README §3.2 把现网 config\ 整目录迁入（缺 config 时引擎会用 example 占位自播种——这不是想要的现网数据，故此处直接拦下）"
}
if (-not (Test-Path (Join-Path $DataRoot 'config\config.local.yaml'))) {
    Write-Host "[start-$InstanceId] 提示: 缺 config.local.yaml（品牌/端口 overlay），将按 config.yaml 原值启动；模板见 deploy\instances\zhiliao\config.local.yaml" -ForegroundColor Yellow
}
if (-not (Test-Path (Join-Path $DataRoot 'domains'))) {
    Fail "实例域包目录缺失: $DataRoot\domains`n  → New-Item -ItemType Junction -Path `"$DataRoot\domains`" -Target `"$EngineDir\domains`"（README §3 第 4 步；缺它域包/人设/KB 种子不加载）"
}
...
#   EVENT_SPOOL_DIR        事件 spool（platform/observability 契约；本实例产品号=zhiliao）
#   CHENGJIE_PRODUCT_ID    事件 product_id=zhiliao（telemetry.py 缺省即 zhiliao，仍显式
#                          设置防外部环境残留串产品号，与通译脚本对称）
...
# Win32_Process.Create 的环境继承在部分账户下不含机器级变量 → 显式透传，确保引擎能发告警。
