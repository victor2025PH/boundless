...
param(
    ...
    [string[]]$Tasks = @(),         # 空 = 全部任务类型；可选 uploader/purge/export/grants_sync/config_snapshot/watchdog/kpi_weekly（逗号分隔亦可）
...
if ($bad.Count) { Say "错误: 未知任务类型 $($bad -join ', ')（可选 uploader/purge/export/grants_sync/config_snapshot/watchdog/kpi_weekly）"; exit 1 }
