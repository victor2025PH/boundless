...
# ── 本机常量（装机时按机器改这一段）─────────────────────────────────────────
$Owner   = "jun"                                   # 值守识别用：jun / skuio
$Fp      = "B990-0C62-FC1C-F668"                   # 机器码
...
if ($Task -eq "reupload") {
    ...
    Write-Output "CODE=UPLOAD_FAILED (上传失败（网络），报告已保存在 $($last.FullName)，可把 .md 直接发群；原因: $script:LastUploadError)"
    exit 2
}
...
if ($code) { Write-Output "CODE=$code"; exit 0 }
Write-Output "CODE=UPLOAD_FAILED (上传失败（网络），报告已保存在 $mdPath，可把 .md 直接发群；网络恢复后可跑 -Task reupload 重传；原因: $script:LastUploadError)"
exit 2
