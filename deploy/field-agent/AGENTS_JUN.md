# 智聊现场取证助手 · 钧机（field-agent v1.3.1，2026-09-08）
...
>
> v1.3.1 变化（2026-09-08，随脚本 v1.3.1 重发）：① 语音类问题关键词统一抓 `[tts]`（1.0.77 起合成 / 终审 /
> 回落 / 配置闸全部带这个前缀，一词抓全）；② **时间窗必须盖住出问题那一刻**——`-Since` 按「问题发生到现在
> 的分钟数 + 10」算，不是固定 30/40；③ 脚本本体 v1.3.1：上传失败自动重试 1+3 次，仍失败用 `-Task reupload`
> 单独重传，不必重跑取证。**本手册与脚本都是覆盖旧文件，不是新增。**

## 0. 本机事实（写死，勿猜）

- 机器码：`B990-0C62-FC1C-F668`
- 应用数据根：`C:\Users\59435\AppData\Roaming\telegram-ai-desktop\data`
- **主日志**：`C:\Users\59435\AppData\Roaming\telegram-ai-desktop\logs\backend.log`（data 的兄弟目录）
